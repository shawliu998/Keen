...
export const learningFeedTaskSchema = z.object({
  ...
  if ((task.status === "completed") !== (task.completed_at !== null)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Task completion state is inconsistent.", path: ["completed_at"] });
  }
});
...
export const learningSnapshotSchema = z.object({
  ...
  completed_tasks: z.array(learningFeedTaskSchema).max(50),
  ...
  snapshot.completed_tasks.forEach((task, index) => {
    if (task.course_id !== snapshot.course_id) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Every completed task must belong to the snapshot course.", path: ["completed_tasks", index, "course_id"] });
    }
    if (task.status !== "completed" || task.completed_at === null) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Completed tasks must have completed status and completed_at.", path: ["completed_tasks", index] });
    }
    if (taskIds.has(task.id)) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Snapshot task IDs must be unique.", path: ["completed_tasks", index, "id"] });
    }
    const completedAt = task.completed_at === null ? Number.NaN : Date.parse(task.completed_at);
    if (completedAt > previousCompletedAt) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Completed tasks must be newest first.", path: ["completed_tasks", index, "completed_at"] });
    }
...
export const autonomousRecommendationRequestSchema = z.object({
  ...
  time_zone: z.string().min(1).max(100).regex(/^[A-Za-z0-9._+-]+(?:\/[A-Za-z0-9._+-]+)*$/).refine((value) => {
    try {
  ...
  }, "time_zone must name an available IANA time zone").default("UTC"),
}).strict();
...
export const autonomousRecommendationResponseSchema = z.object({
  ...
  if (result.task !== null) {
    ...
    if (result.task.status === "completed") {
      ...
      const completedTasks = result.snapshot.completed_tasks.filter((task) => task.id === result.task?.id);
      if (completedTasks.length !== 1 || !sameValidatedValue(completedTasks[0], result.task)) {
        context.addIssue({ code: z.ZodIssueCode.custom, message: "A completed recommendation task must exactly match its completed snapshot task.", path: ["task"] });
      }
...
export const autonomousStudySessionSchema = z.object({
  ...
  resume_from_status: z.enum(["draft", "goal_confirmation", "diagnosing", "planning", "studying", "checkpoint", "active_recall", "practicing", "summarizing", "review_scheduling"]).nullable().optional(),
  ...
  const hasResumeState = session.resume_from_status !== null && session.resume_from_status !== undefined;
  if ((session.status === "paused") !== hasResumeState) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A paused session must expose exactly one resume state.", path: ["resume_from_status"] });
  }
});

export const studySessionControlRequestSchema = z.object({
  course_id: learningIdentifierSchema,
  expected_revision: z.number().int().nonnegative(),
  idempotency_key: z.string().min(16).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/),
}).strict();

export const studySessionControlResponseSchema = z.object({
  ...
  if (result.session.course_id !== result.course_id) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A session control result must stay inside its course.", path: ["session", "course_id"] });
  }
  if (result.outcome === "applied" && result.command === "pause" && result.session.status !== "paused") {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "An applied pause must return a paused session.", path: ["session", "status"] });
  }
  if (result.outcome === "applied" && result.command === "resume" && result.session.status === "paused") {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "An applied resume must return an active session phase.", path: ["session", "status"] });
  }
});
...
export const focusedStudyRequestSchema = z.object({
  course_id: learningIdentifierSchema,
  goal: z.string()
    .refine((value) => !/\p{C}/u.test(value), "goal must not contain control characters")
...
export const focusedStudyResponseSchema = z.object({
  outcome: z.enum(["session_created", "replayed", "blocked"]),
  course_id: learningIdentifierSchema,
  task: autonomousStudyTaskSchema.nullable(),
  session: autonomousStudySessionSchema.nullable(),
  plan: autonomousStudyPlanSchema.nullable(),
  blocked_reason: z.enum(["course_not_found", "no_matching_indexed_source"]).nullable(),
  recovery_action: z.string().min(1).max(500).nullable(),
}).strict().superRefine((result, context) => {
  if (result.outcome === "blocked") {
    if (result.session !== null || result.plan !== null || result.blocked_reason === null || result.recovery_action === null) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "A blocked focused-study request must expose recovery without a session or plan." });
    }
    if (result.task !== null && result.task.course_id !== result.course_id) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "A visible blocked task must belong to the focused-study course.", path: ["task", "course_id"] });
    }
    return;
  }
  if (result.task === null || result.session === null || result.blocked_reason !== null || result.recovery_action !== null) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A successful focused-study request must return its task and session without blocked recovery." });
    return;
  }
  if (result.task.course_id !== result.course_id || result.session.course_id !== result.course_id) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Focused-study entities must share one course scope." });
  }
  if (result.session.originating_task_id !== result.task.id) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A focused-study session must retain its originating task.", path: ["session", "originating_task_id"] });
  }
  if (result.plan !== null && result.plan.session_id !== result.session.id) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A returned focused-study plan must belong to its session.", path: ["plan", "session_id"] });
  }
});
...
export const studySessionReadResponseSchema = z.object({
  ...
  if (result.outcome === "ready") {
    ...
    const currentUnit = result.current_unit_id === null ? null : result.plan.units.find((unit) => unit.id === result.current_unit_id) ?? null;
    if (activeUnits.length > 1) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "A study plan may expose at most one active unit.", path: ["plan", "units"] });
    }
    if ((currentUnit === null) !== (activeUnits.length === 0) || (currentUnit !== null && (currentUnit.status !== "active" || activeUnits[0]?.id !== currentUnit.id))) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "The current-unit pointer must identify the plan's unique active unit.", path: ["current_unit_id"] });
    }
    const effectiveStatus = result.session.status === "paused" ? result.session.resume_from_status : result.session.status;
    const requiresCurrentUnit = effectiveStatus != null && ["studying", "checkpoint", "active_recall", "practicing", "summarizing", "review_scheduling"].includes(effectiveStatus);
    if (requiresCurrentUnit && currentUnit === null) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "The current session phase requires one active current unit.", path: ["current_unit_id"] });
    }
    if (["completed", "cancelled", "failed"].includes(result.session.status) && currentUnit !== null) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "A terminal study session cannot expose an active current unit.", path: ["current_unit_id"] });
    }
...
export const adaptiveActionSchema = z.object({
  ...
  course_id: learningIdentifierSchema,
  session_id: learningIdentifierSchema,
  unit_id: z.string().min(1).max(256),
  kind: z.enum(["remediate", "practice"]),
  status: z.enum(["pending", "completed", "cancelled"]),
  reason_code: z.enum(["active_recall_correct", "active_recall_incorrect", "remediation_completed"]),
  policy_version: z.literal("adaptive-session-policy/1.0.0"),
  revision: z.number().int().nonnegative(),
  created_at: isoDateTimeSchema,
  started_at: isoDateTimeSchema.nullable(),
  completed_at: isoDateTimeSchema.nullable(),
  ...
  const terminalDatesAreValid = action.status === "pending"
    ? action.completed_at === null && action.cancelled_at === null
    : action.status === "completed"
      ? action.completed_at !== null && action.cancelled_at === null
      : action.completed_at === null && action.cancelled_at !== null;
  if (!terminalDatesAreValid) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive action status must match its terminal timestamps.", path: ["status"] });
  }
  if ((action.kind === "remediate") !== (action.reason_code === "active_recall_incorrect")) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive action kind must match its persisted reason.", path: ["reason_code"] });
  }
  for (const [field, value] of [["started_at", action.started_at], ["completed_at", action.completed_at], ["cancelled_at", action.cancelled_at]] as const) {
    if (value !== null && timestampIsAfter(action.created_at, value)) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive action timestamps are inconsistent.", path: [field] });
    }
  }
});

const adaptiveStateFields = {
  course_id: learningIdentifierSchema,
  session: autonomousStudySessionSchema,
  plan: autonomousStudyPlanSchema.nullable(),
  current_unit: autonomousStudySessionUnitSchema.nullable(),
  current_unit_id: z.string().min(1).max(256).nullable(),
  action: adaptiveActionSchema.nullable(),
};

function verifyAdaptiveScope(
  result: {
    course_id: string;
    session: z.infer<typeof autonomousStudySessionSchema>;
    plan: z.infer<typeof autonomousStudyPlanSchema> | null;
    current_unit: z.infer<typeof autonomousStudySessionUnitSchema> | null;
    current_unit_id: string | null;
    action: z.infer<typeof adaptiveActionSchema> | null;
  },
  context: z.RefinementCtx,
): void {
  if (result.session.course_id !== result.course_id) context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive session must belong to the response course.", path: ["session", "course_id"] });
  if (result.plan !== null && result.plan.session_id !== result.session.id) context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive plan must belong to the response session.", path: ["plan", "session_id"] });
  if ((result.current_unit === null) !== (result.current_unit_id === null) || (result.current_unit !== null && result.current_unit.id !== result.current_unit_id)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive current unit and pointer must match.", path: ["current_unit_id"] });
  }
  if (result.current_unit !== null) {
    const planUnit = result.plan?.units.find((unit) => unit.id === result.current_unit_id);
    if (planUnit === undefined || !sameValidatedValue(planUnit, result.current_unit)) context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive current unit must exactly match a plan unit.", path: ["current_unit"] });
  }
  if (result.action !== null && (
    result.action.course_id !== result.course_id
    || result.action.session_id !== result.session.id
    || result.plan === null
    || !result.plan.units.some((unit) => unit.id === result.action?.unit_id)
    || (result.current_unit_id !== null && result.action.unit_id !== result.current_unit_id)
  )) context.addIssue({ code: z.ZodIssueCode.custom, message: "Adaptive action must stay inside the current course, session, and unit.", path: ["action"] });
}

export const adaptiveStateResponseSchema = z.discriminatedUnion("state", [
  z.object({ state: z.literal("legacy_canonical"), ...adaptiveStateFields }).strict(),
  z.object({ state: z.literal("canonical"), ...adaptiveStateFields }).strict(),
  z.object({ state: z.literal("action_required"), ...adaptiveStateFields }).strict(),
  z.object({ state: z.literal("paused"), ...adaptiveStateFields }).strict(),
  z.object({ state: z.literal("terminal"), ...adaptiveStateFields }).strict(),
]).superRefine((result, context) => {
  verifyAdaptiveScope(result, context);
  if (result.state === "legacy_canonical" && result.action !== null) context.addIssue({ code: z.ZodIssueCode.custom, message: "Legacy canonical state cannot claim a persisted adaptive action.", path: ["action"] });
  if (result.state === "action_required" && result.action?.status !== "pending") context.addIssue({ code: z.ZodIssueCode.custom, message: "Action-required state needs one pending adaptive action.", path: ["action"] });
  if ((result.state === "canonical" || result.state === "terminal") && result.action !== null) context.addIssue({ code: z.ZodIssueCode.custom, message: "Canonical and terminal adaptive states cannot expose an action.", path: ["action"] });
  if (result.state === "paused" && result.session.status !== "paused") context.addIssue({ code: z.ZodIssueCode.custom, message: "Paused adaptive state needs a paused session.", path: ["session", "status"] });
  if (result.state === "terminal" && !["completed", "cancelled", "failed"].includes(result.session.status)) context.addIssue({ code: z.ZodIssueCode.custom, message: "Terminal adaptive state needs a terminal session.", path: ["session", "status"] });
});

export const adaptiveActionCompleteRequestSchema = z.object({
  course_id: learningIdentifierSchema,
  expected_action_revision: z.number().int().nonnegative(),
  idempotency_key: z.string().min(16).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/),
}).strict();

export const adaptiveActionCompleteResponseSchema = z.object({
  outcome: z.enum(["applied", "replayed"]),
  course_id: learningIdentifierSchema,
  session: autonomousStudySessionSchema,
  plan: autonomousStudyPlanSchema,
  completed_action: adaptiveActionSchema,
  current_action: adaptiveActionSchema,
}).strict().superRefine((result, context) => {
  verifyAdaptiveScope({ ...result, current_unit: null, current_unit_id: null, action: result.completed_action }, context);
  if (result.completed_action.status !== "completed" || result.completed_action.kind !== "remediate") context.addIssue({ code: z.ZodIssueCode.custom, message: "Completed adaptive action must be a completed remediation.", path: ["completed_action", "status"] });
  const pendingPractice = result.current_action.status === "pending" && result.current_action.kind === "practice";
  const consumedPracticeReplay = result.outcome === "replayed" && result.current_action.status === "completed" && result.current_action.kind === "practice";
  if (!pendingPractice && !consumedPracticeReplay) context.addIssue({ code: z.ZodIssueCode.custom, message: "Current adaptive action must be pending practice, except for its completed late-replay projection.", path: ["current_action"] });
  if (result.current_action.reason_code !== "remediation_completed" || result.current_action.policy_version !== result.completed_action.policy_version) context.addIssue({ code: z.ZodIssueCode.custom, message: "Current practice action must be the remediation successor under the same policy.", path: ["current_action", "reason_code"] });
  if (result.current_action.course_id !== result.course_id || result.current_action.session_id !== result.session.id || result.current_action.unit_id !== result.completed_action.unit_id || !result.plan.units.some((unit) => unit.id === result.current_action.unit_id)) context.addIssue({ code: z.ZodIssueCode.custom, message: "The next adaptive action must remain in the completed action scope.", path: ["current_action"] });
});

export const studySessionListResponseSchema = z.object({
  course_id: learningIdentifierSchema,
  sessions: z.array(autonomousStudySessionSchema).max(100),
}).strict().superRefine((result, context) => {
  result.sessions.forEach((session, index) => {
    if (session.course_id !== result.course_id) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Study history sessions must belong to the requested course.", path: ["sessions", index, "course_id"] });
    }
  });
});
...
function verifyDiagnosticSessionStatus(
  ...
  const status = session.status === "paused" ? session.resume_from_status : session.status;
  const matches = expected === "answered"
    ? status !== null && status !== undefined && answeredDiagnosticSessionStatuses.has(status)
    : status === expected;
  if (!matches) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
...
export const diagnosticReadResponseSchema = z.discriminatedUnion("outcome", [
  ...
  if (result.outcome === "not_started") {
    verifyDiagnosticSessionStatus(result.session, "goal_confirmation", context);
    verifyNotStartedDiagnostic(result.plan, context);
  }
  if (result.outcome === "pending") {
    verifyDiagnosticSessionStatus(result.session, "diagnosing", context);
    verifyPendingDiagnostic(result, context);
  }
  if (result.outcome === "answered") {
    verifyDiagnosticSessionStatus(result.session, "answered", context);
...
function verifyTargetedPracticeState(result: TargetedPracticeState, context: z.RefinementCtx): void {
  ...
  if (result.plan.units.filter((unit) => unit.status === "active").length > 1) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A practice plan may expose at most one active unit.", path: ["plan", "units"] });
  }
}
...
export const targetedPracticeProgressionResponseSchema = z.object({
  ...
  const outcome = result.run.status === "pending" ? "pending" : result.run.status === "answered" ? "answered" : "cancelled";
  const appliedAnswer = result.outcome === "applied" && outcome === "answered";
  verifyTargetedPracticeOutcome({ ...result, outcome }, context, result.outcome === "replayed", appliedAnswer);
  if (!appliedAnswer) return;
  const current = result.current_unit;
  const units = result.plan.units;
  if (current === null || current.status !== "active") {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "An applied practice answer must retain or advance to one active unit.", path: ["current_unit"] });
    return;
  }
  const earlier = units.slice(0, current.ordinal);
  const later = units.slice(current.ordinal + 1);
  if (earlier.some((unit) => unit.status !== "completed" && unit.status !== "skipped")) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Units before the active unit must be completed or skipped.", path: ["plan", "units"] });
  }
  if (later.some((unit) => unit.status !== "locked" && unit.status !== "ready")) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Units after the active unit must remain locked or ready.", path: ["plan", "units"] });
  }
  if (result.session.status === "studying") {
    if (current.ordinal === 0 || units[current.ordinal - 1]?.status !== "completed") {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "A studying practice answer must advance from a completed unit to its successor.", path: ["current_unit"] });
    }
  } else if (current.ordinal !== units.length - 1) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Only the final active unit may enter summarizing.", path: ["session", "status"] });
  }
});
...
export const reviewItemSchema = z.object({
  id: learningIdentifierSchema,
  course_id: learningIdentifierSchema,
  course_title: z.string().min(1).max(240),
  concept_id: learningIdentifierSchema,
  concept_name: z.string().min(1).max(240),
  item_type: z.enum(["flashcard", "free_recall", "concept_explanation", "error_replay", "practice_problem"]),
  prompt: z.string().min(1).max(8_000),
  expected_answer: z.unknown(),
  source_type: z.enum(["mastery_evidence", "misconception", "assessment", "study_session", "manual", "agent_recommendation"]),
  source_id: learningIdentifierSchema.nullable(),
  due_at: isoDateTimeSchema,
...
export const dueReviewListResponseSchema = z.object({
  as_of: isoDateTimeSchema,
  items: z.array(reviewItemSchema).max(50),
}).strict().superRefine((result, context) => {
  result.items.forEach((item, index) => {
    if (timestampIsAfter(item.due_at, result.as_of)) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "A due-review response cannot include future items.", path: ["items", index, "due_at"] });
    }
  });
});
...
export const reviewAttemptRequestSchema = z.object({
  ...
  taskContext: z.object({
    taskId: learningIdentifierSchema,
    courseId: learningIdentifierSchema,
  }).strict().optional(),
  idempotencyKey: z.string().min(16).max(200).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/),
}).strict();

export const reviewAttemptResponseSchema = z.object({
  outcome: z.enum(["applied", "replayed"]),
  review_item_id: learningIdentifierSchema,
  rating: z.enum(["again", "hard", "good", "easy"]),
  response: z.string().max(8_000),
  ...
  if (timestampIsAfter(result.reviewed_at, result.schedule.due_at)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A review schedule cannot be due before it was rated.", path: ["schedule", "due_at"] });
  }
  if (result.reviewed_at !== result.schedule.last_reviewed_at) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "A review schedule must retain its rating timestamp.", path: ["schedule", "last_reviewed_at"] });
  }
});
...
export const answerRetrievalChunkSchema = z.object({
  ...
  chunkIds: z.array(z.string().min(1)).min(1),
  documentId: z.string().min(1),
  documentVersionId: z.string().min(1).max(128).optional(),
  chunkContentHash: z.string().regex(/^[a-f0-9]{64}$/).optional(),
  documentName: z.string().min(1),
  pageNumber: z.number().int().positive(),
  pageEnd: z.number().int().positive(),
  sectionPath: z.array(z.string()),
  ...
  if (chunk.chunkIds[0] !== chunk.chunkId || new Set(chunk.chunkIds).size !== chunk.chunkIds.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "chunkIds must be unique and begin with chunkId.", path: ["chunkIds"] });
  }
...
export const conversationSourceScopeSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("all_indexed") }).strict(),
  z.object({ kind: z.literal("course"), courseId: z.string().min(1).max(128) }).strict(),
]);

export const conversationSchema = z.object({
  id: z.string().uuid(),
  courseId: z.string().min(1).max(128).nullable(),
  title: z.string().min(1).max(500),
  mode: z.literal("ask"),
  status: z.enum(["active", "archived"]),
  sourceScope: conversationSourceScopeSchema,
  createdAt: isoDateTimeSchema,
  updatedAt: isoDateTimeSchema,
  archivedAt: isoDateTimeSchema.nullable(),
}).strict().superRefine((conversation, context) => {
  const scopedCourseId = conversation.sourceScope.kind === "course" ? conversation.sourceScope.courseId : null;
  if (conversation.courseId !== scopedCourseId) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Conversation courseId must match sourceScope.", path: ["courseId"] });
  }
  if ((conversation.status === "archived") !== (conversation.archivedAt !== null)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Conversation archive fields are inconsistent.", path: ["archivedAt"] });
  }
});
...
export const conversationSummarySchema = z.object({
  ...
  status: z.enum(["active", "archived"]),
  sourceScope: conversationSourceScopeSchema,
  courseTitle: z.string().min(1).max(500).nullable(),
  messageCount: z.number().int().nonnegative(),
  lastMessagePreview: z.string().max(160).nullable(),
  answerStatus: z.enum(["pending", "streaming", "completed", "failed", "cancelled", "interrupted"]).nullable(),
  createdAt: isoDateTimeSchema,
  updatedAt: isoDateTimeSchema,
}).strict().superRefine((conversation, context) => {
  if (conversation.sourceScope.kind === "all_indexed" && conversation.courseTitle !== null) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "All-indexed conversations cannot carry a course title.", path: ["courseTitle"] });
  }
});
...
export const conversationListResponseSchema = z.object({
  ...
  response.conversations.forEach((conversation, index) => {
    if (identities.has(conversation.id)) context.addIssue({ code: z.ZodIssueCode.custom, message: "Conversation ids must be unique.", path: ["conversations", index, "id"] });
    identities.add(conversation.id);
    if (index > 0) {
      const previous = response.conversations[index - 1];
      if (previous && (Date.parse(previous.updatedAt) < Date.parse(conversation.updatedAt)
        || (previous.updatedAt === conversation.updatedAt && previous.id.localeCompare(conversation.id) < 0))) {
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Conversations must use stable descending history order.", path: ["conversations", index] });
      }
    }
  });
});

export const durableAnswerCitationSchema = z.object({
  citationId: z.string().min(1).max(128),
  sourceIndex: z.number().int().positive(),
  chunkId: z.string().min(1).max(128),
  documentId: z.string().min(1).max(128),
  documentVersionId: z.string().min(1).max(128),
  chunkContentHash: z.string().regex(/^[a-f0-9]{64}$/),
  documentName: z.string().min(1),
  pageNumber: z.number().int().positive(),
  sectionPath: z.array(z.string()),
  excerpt: z.string().min(1),
  ...
  const bbox = citation.bbox;
  if (!bbox) return;
  if (bbox.x0 < 0 || bbox.y0 < 0 || bbox.x1 <= bbox.x0 || bbox.y1 <= bbox.y0 || bbox.x1 > bbox.pageWidth || bbox.y1 > bbox.pageHeight) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Citation bbox must be ordered within its PDF page bounds.", path: ["bbox"] });
  }
});

export const durableConversationMessageSchema = z.object({
  id: z.string().uuid(),
  conversationId: z.string().uuid(),
  sequence: z.number().int().nonnegative(),
  role: z.enum(["user", "assistant"]),
  status: z.enum(["pending", "streaming", "completed", "failed", "cancelled", "interrupted"]),
  content: z.string(),
  replyToMessageId: z.string().uuid().nullable(),
  sourceScope: conversationSourceScopeSchema,
  retrievalLimit: z.number().int().min(6).max(10).nullable(),
  ...
  createdAt: isoDateTimeSchema,
  updatedAt: isoDateTimeSchema,
  citations: z.array(durableAnswerCitationSchema),
}).strict().superRefine((message, context) => {
  if ((message.role === "user") !== (message.replyToMessageId === null)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Only assistant messages reply to another message.", path: ["replyToMessageId"] });
  }
  if ((message.role === "user") !== (message.retrievalLimit === null)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Only assistant messages carry a retrieval limit.", path: ["retrievalLimit"] });
  }
  if (message.status === "completed" && message.finishedAt === null) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Completed messages require finishedAt.", path: ["finishedAt"] });
  }
  if (message.role === "user" && message.citations.length > 0) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "User messages cannot contain answer citations.", path: ["citations"] });
  }
});

export const durableConversationMessagesResponseSchema = z.object({
  messages: z.array(durableConversationMessageSchema),
}).strict().superRefine((response, context) => {
  const ids = new Set<string>();
  response.messages.forEach((message, index) => {
    if (ids.has(message.id)) context.addIssue({ code: z.ZodIssueCode.custom, message: "Message ids must be unique.", path: ["messages", index, "id"] });
    ids.add(message.id);
    if (message.sequence !== index) context.addIssue({ code: z.ZodIssueCode.custom, message: "Messages must be in contiguous sequence order.", path: ["messages", index, "sequence"] });
  });
});

export const durableAnswerStreamRequestSchema = z.object({
  question: z.string().trim().min(1).max(8_000),
  idempotencyKey: z.string().min(16).max(200).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/),
  userMessageId: z.string().uuid(),
  assistantMessageId: z.string().uuid(),
  sourceScope: conversationSourceScopeSchema,
  retrievalLimit: z.number().int().min(6).max(10),
}).strict().superRefine((request, context) => {
  if (request.userMessageId === request.assistantMessageId) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "User and assistant message ids must differ.", path: ["assistantMessageId"] });
  }
});

export const conversationCancelRequestSchema = z.object({ idempotencyKey: z.string().min(16).max(200).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/) }).strict();
export const conversationCancelResponseSchema = z.object({
  message: durableConversationMessageSchema,
  replayed: z.boolean(),
}).strict();
...
export type StudySessionControlRequest = z.input<typeof studySessionControlRequestSchema>;
export type StudySessionControlResponse = z.infer<typeof studySessionControlResponseSchema>;
export type AutonomousStudySessionStartResponse = z.infer<typeof autonomousStudySessionStartResponseSchema>;
export type FocusedStudyRequest = z.input<typeof focusedStudyRequestSchema>;
export type FocusedStudyResponse = z.infer<typeof focusedStudyResponseSchema>;
...
export type AdaptiveStateResponse = z.infer<typeof adaptiveStateResponseSchema>;
export type AdaptiveActionCompleteRequest = z.input<typeof adaptiveActionCompleteRequestSchema>;
export type AdaptiveActionCompleteResponse = z.infer<typeof adaptiveActionCompleteResponseSchema>;
export type StudySessionListResponse = z.infer<typeof studySessionListResponseSchema>;
...
export type DueReviewListResponse = z.infer<typeof dueReviewListResponseSchema>;
export type ReviewAttemptRequest = z.input<typeof reviewAttemptRequestSchema>;
export type ReviewAttemptResponse = z.infer<typeof reviewAttemptResponseSchema>;
...
export type ConversationSourceScope = z.infer<typeof conversationSourceScopeSchema>;
...
export type DurableAnswerCitation = z.infer<typeof durableAnswerCitationSchema>;
export type DurableConversationMessage = z.infer<typeof durableConversationMessageSchema>;
export type DurableConversationMessagesResponse = z.infer<typeof durableConversationMessagesResponseSchema>;
export type DurableAnswerStreamRequest = z.infer<typeof durableAnswerStreamRequestSchema>;
