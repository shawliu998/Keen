  ...
  z.object({
    ...
    data: z.union([
      ...
      z.object({
        ...
        code: z.enum(["invalid_arguments", "temporary_read_failure"]),
...
export const agentRunActivityContextSchema = z.union([
  z.object({ conversationId: agentIdentifierSchema }).strict(),
  z.object({ studySessionId: agentIdentifierSchema }).strict(),
]);

export const agentRunActivityEventSchema = z.object({
  id: agentIdentifierSchema,
  ...
  const parsed = agentRunEventSchema.safeParse({ id: event.id, type: event.type, data: event.data });
  if (!parsed.success) {
    parsed.error.issues.forEach((issue) => context.addIssue(issue));
    return z.NEVER;
  }
  return { ...parsed.data, sequence: event.sequence, createdAt: event.createdAt };
});

export const agentRunActivitySchema = z.object({
  run: agentRunSchema.nullable(),
  events: z.array(agentRunActivityEventSchema),
}).strict().superRefine((activity, context) => {
  if (activity.run === null && activity.events.length > 0) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Empty Agent activity must not include events.", path: ["events"] });
  }
  const eventIds = new Set<string>();
  activity.events.forEach((event, index) => {
    if (eventIds.has(event.id)) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Agent activity event IDs must be unique.", path: ["events", index, "id"] });
    }
    eventIds.add(event.id);
    if (event.sequence !== index) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Agent activity events must contain the complete durable sequence from zero.", path: ["events", index, "sequence"] });
    }
  });
  const metadataEvents = activity.events
    .map((event, index) => ({ event, index }))
    .filter(({ event }) => event.type === "metadata");
  if (metadataEvents.length > 1) {
    metadataEvents.slice(1).forEach(({ index }) => {
      context.addIssue({ code: z.ZodIssueCode.custom, message: "Agent activity may contain only one metadata event.", path: ["events", index, "type"] });
    });
  }
  const metadata = metadataEvents[0];
  if (metadata && metadata.index !== 0) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Agent activity metadata must be the first durable event.", path: ["events", metadata.index, "sequence"] });
  }
  if (activity.run !== null && metadata?.event.type === "metadata" && metadata.event.data.runId !== activity.run.id) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Agent activity metadata must identify the returned run.", path: ["events", metadata.index, "data", "runId"] });
  }
});
...
export type AgentRunActivityContext = z.input<typeof agentRunActivityContextSchema>;
export type AgentRunActivityEvent = z.infer<typeof agentRunActivityEventSchema>;
