...
| Deep Learn | Live Start/Resume renders a persisted plan, opening diagnostic, Active Recall and targeted practice. Pending/unresolved recall or practice hides lesson/session/source clues before paint; answers record deterministic mastery evidence once. Browser Demo remains explicitly sample-only and makes zero requests. | editable plan, teaching, targeted-practice screenshot/accessibility coverage, additional checkpoint/item types, summary, Review/FSRS handoff, broader persisted progress/pause/resume/recovery and all service/provider failures |
