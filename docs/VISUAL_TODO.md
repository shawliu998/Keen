...
| Deep Learn | Live Start/Resume renders a persisted plan, opening diagnostic and one persisted Active Recall cloze. Pending/unresolved recall hides lesson/session/source clues before paint; answer records deterministic mastery evidence once. Targeted-practice backend/HTTP is implemented but has no TypeScript/UI client or visual state. Browser Demo remains explicitly sample-only and makes zero requests. | editable plan, teaching, targeted-practice UI and recovery states, additional checkpoint/item types, summary, Review/FSRS handoff, broader persisted progress/pause/resume/recovery and all service/provider failures |
...
The 2026-07-18 targeted-practice backend/HTTP slice adds no rendered client state. Targeted-practice UI, loading, recovery, paused and answered visual coverage remain outstanding; no screenshot, accessibility audit or authorized visual diff exists, so no visual implementation or parity claim is made.
