...
| Python | FastAPI, Pydantic, Uvicorn, pypdf 6.14.2, python-multipart 0.0.32; dev: httpx, pytest, pytest-cov; build: PyInstaller 6.21.0 and community hooks 2026.6 | Resolved `.venv` report exists, but no committed Python lockfile | Exact pypdf/python-multipart wheels and installed PyInstaller/hook license files were inspected as recorded below. The earlier environment-wide `pip-licenses`/`pip-audit` evidence is not a reproducible runtime lock or a complete shipped notice set. |
...
No upstream source file was copied for these integrations. pypdf and python-multipart are ordinary runtime package dependencies; PyInstaller and its community hooks form the pinned build boundary, while the generated sidecar contains PyInstaller bootloader/loader material and dependency bytecode. These records do not make the distribution notice bundle complete.
