...
# Tauri's DMG bundler removes its intermediate macOS app after creating the
# image. Build the standalone app last so one packaging run leaves both RC
# artifacts available for local installation and direct launch verification.
npm run tauri --workspace=@keen/desktop -- build \
  --bundles app \
  --config "${TAURI_SIDECAR_CONFIG}"

"${SCRIPT_DIR}/smoke-bundled-sidecar.sh"
"${SCRIPT_DIR}/smoke-bundled-review-restart.py"
"${SCRIPT_DIR}/smoke-bundled-provider-journey.py"
