use crate::{
    keychain, provider_config,
    security::{canonicalize_within, generate_session_token},
};
use serde::Serialize;
#[cfg(unix)]
...
impl SidecarArguments {
    fn into_os_strings(self) -> Vec<OsString> {
        let mut arguments = vec![
            OsString::from("--sidecar-secrets-stdin"),
            OsString::from("--port"),
            OsString::from("0"),
        ...
        if let Some(chat) = self.chat {
            let has_api_key = chat.api_key.is_some();
            arguments.extend([
            ...
            if has_api_key {
                arguments.push(OsString::from("--chat-api-key-frame"));
            }
        }
        arguments
    }
...
impl SidecarSupervisor {
    ...
    pub fn restart<R: Runtime>(
        ...
        self.restart_inner(app, reason, false)
    }

    pub fn restart_for_configuration<R: Runtime>(
        &self,
        app: AppHandle<R>,
    ) -> Result<SidecarConnectionView, String> {
        self.restart_inner(app, Some("provider_configuration_changed".to_owned()), true)
    }

    fn restart_inner<R: Runtime>(
        &self,
        app: AppHandle<R>,
        reason: Option<String>,
        replace_in_progress_launch: bool,
    ) -> Result<SidecarConnectionView, String> {
        if self.shutdown_is_requested() {
        ...
        let preparation = {
            let mut state = self.lock();
            prepare_manual_restart_state(&mut state, reason, replace_in_progress_launch)
        };
    ...
    > {
        ...
        let chat = prefer_saved_chat_configuration(
            chat_arguments_from_saved_configuration(&database_path)?,
            chat_arguments_from_environment,
        )?;
        let chat_api_key = chat.as_ref().and_then(|value| value.api_key.clone());
        ...
        let spawn_result = match launch_program {
            LaunchProgram::Bundled => {
                ...
                let command = command
                    ...
                    .args(arguments);
                spawn_managed_process(command.into(), &token, chat_api_key.as_deref())
...
fn spawn_managed_process(
    ...
    token: &str,
    api_key: Option<&std::ffi::OsStr>,
) -> io::Result<(mpsc::Receiver<ManagedProcessEvent>, ManagedProcess)> {
    configure_managed_process(&mut command, token, api_key)?;
...
fn configure_managed_process(
    ...
    write_secret_frame(&mut token_writer, token.as_bytes())?;
...
fn write_secret_frame(writer: &mut impl Write, value: &[u8]) -> io::Result<()> {
...
#[cfg(not(unix))]
fn configure_managed_process(
    _command: &mut StdCommand,
    _token: &str,
    _api_key: Option<&std::ffi::OsStr>,
) -> io::Result<()> {
    Err(io::Error::new(
...
fn launch_program() -> Result<LaunchProgram, SidecarError> {
    ...
    let executable = std::env::current_exe().map_err(SidecarError::DevelopmentLaunch)?;
    launch_program_for_paths(&executable, &repository_root, true)
}

fn launch_program_for_paths(
    executable: &Path,
    repository_root: &Path,
    debug_build: bool,
) -> Result<LaunchProgram, SidecarError> {
    if !debug_build || executable_is_in_macos_app_bundle(executable) {
        return Ok(LaunchProgram::Bundled);
    }

    let python = repository_root.join(".venv/bin/python");
    let service_root = repository_root.join("services/learning-core");
    let entrypoint = repository_root.join("apps/desktop/src-tauri/sidecar/entrypoint.py");
    if !python.is_file() || !service_root.is_dir() || !entrypoint.is_file() {
...
fn executable_is_in_macos_app_bundle(executable: &Path) -> bool {
...
fn chat_arguments_from_saved_configuration(
    database_path: &Path,
) -> Result<Option<ChatArguments>, SidecarError> {
    let Some(parent) = database_path.parent() else {
        return Err(SidecarError::ChatConfiguration);
    };
    let path = provider_config::config_path(parent);
    let Some(configuration) =
        provider_config::load(&path).map_err(|_| SidecarError::ChatConfiguration)?
    else {
        return Ok(None);
    };
    let api_key = if let Some(origin) = provider_config::remote_key_origin(&configuration)
        .map_err(|_| SidecarError::ChatConfiguration)?
    {
        Some(OsString::from(
            keychain::get(&origin)
                .map_err(|_| SidecarError::ChatConfiguration)?
                .ok_or(SidecarError::ChatConfiguration)?,
        ))
    } else {
        None
    };
    let provider = OsString::from(configuration.provider);
    let base_url = OsString::from(configuration.endpoint);
    let model = OsString::from(configuration.model);
    let version = model.clone();
    if validate_chat_model_provider_arguments(&provider, &base_url, &model, &version).is_err() {
        return Err(SidecarError::ChatConfiguration);
    }
...
fn prefer_saved_chat_configuration<F>(
    saved: Option<ChatArguments>,
    environment_fallback: F,
) -> Result<Option<ChatArguments>, SidecarError>
where
    F: FnOnce() -> Result<Option<ChatArguments>, SidecarError>,
{
    match saved {
        Some(configuration) => Ok(Some(configuration)),
        None => environment_fallback(),
    }
}
...
fn validate_chat_model_provider_arguments(
    provider: &std::ffi::OsStr,
    base_url: &std::ffi::OsStr,
    model: &std::ffi::OsStr,
    version: &std::ffi::OsStr,
) -> Result<(), SidecarError> {
    let provider = provider.to_str().ok_or(SidecarError::ChatConfiguration)?;
    let base_url = base_url.to_str().ok_or(SidecarError::ChatConfiguration)?;
    let model = model.to_str().ok_or(SidecarError::ChatConfiguration)?;
    let version = version.to_str().ok_or(SidecarError::ChatConfiguration)?;
    if !matches!(provider, "ollama" | "openai-compatible")
        || !valid_embedding_identity(model)
        || !valid_embedding_identity(version)
        || !valid_chat_provider_url(provider, base_url)
    {
        return Err(SidecarError::ChatConfiguration);
    }
    Ok(())
...
fn valid_chat_provider_url(provider: &str, value: &str) -> bool {
    ...
    let loopback = provider_config::is_loopback_url_host(&url);
    if provider == "ollama" {
        return url.scheme() == "http" && loopback && url.port().is_some();
    }
    (url.scheme() == "http" && loopback && url.port().is_some()) || url.scheme() == "https"
}
...
fn normalized_restart_reason(reason: Option<&str>) -> &'static str {
    match reason {
        ...
        Some("provider_configuration_changed") => "provider configuration changed",
...
mod tests {
    ...
    #[test]
    fn packaged_debug_executable_uses_bundled_sidecar_without_repository_fallback() {
        let missing_repository = tempfile::tempdir().expect("missing repository fixture");
        let executable = missing_repository
            .path()
            .join("Keen Dev.app/Contents/MacOS/keen-desktop");

        assert!(matches!(
            launch_program_for_paths(&executable, missing_repository.path(), true),
            Ok(LaunchProgram::Bundled)
        ));
    }

    #[test]
    fn non_bundle_debug_executable_uses_development_watchdog_entrypoint() {
        let repository = tempfile::tempdir().expect("development repository fixture");
        let python = repository.path().join(".venv/bin/python");
        let service_root = repository.path().join("services/learning-core");
        let entrypoint = repository
            .path()
            .join("apps/desktop/src-tauri/sidecar/entrypoint.py");
        std::fs::create_dir_all(python.parent().expect("Python parent"))
            .expect("Python fixture directory");
        std::fs::create_dir_all(&service_root).expect("service fixture directory");
        std::fs::create_dir_all(entrypoint.parent().expect("entrypoint parent"))
            .expect("entrypoint fixture directory");
        std::fs::write(&python, b"python fixture").expect("Python fixture");
        std::fs::write(&entrypoint, b"entrypoint fixture").expect("entrypoint fixture");
        let executable = repository.path().join("target/debug/keen-desktop");

        match launch_program_for_paths(&executable, repository.path(), true)
            .expect("development launch program")
        {
            LaunchProgram::Development {
                python: selected_python,
                service_root: selected_service_root,
                entrypoint: selected_entrypoint,
            } => {
                assert_eq!(selected_python, python);
                assert_eq!(selected_service_root, service_root);
                assert_eq!(selected_entrypoint, entrypoint);
            }
            LaunchProgram::Bundled => panic!("non-bundle debug execution must use development"),
        }
    }
    ...
    fn chat_arguments_are_all_or_none_loopback_only_and_forwarded() {
        ...
        assert!(chat_arguments_from_values([
            Some(OsString::from("ollama")),
            Some(OsString::from("http://[::1]:11434")),
            Some(OsString::from("local-chat")),
            Some(OsString::from("v1")),
        ])
        .expect("IPv6 loopback chat configuration")
        .is_some());
    ...
    #[test]
    fn saved_chat_configuration_does_not_validate_stale_environment_values() {
        let saved = ChatArguments {
            provider: OsString::from("ollama"),
            base_url: OsString::from("http://127.0.0.1:11434"),
            model: OsString::from("qwen3"),
            version: OsString::from("saved-provider-configuration"),
            api_key: None,
        };
        let selected =
            prefer_saved_chat_configuration(Some(saved), || Err(SidecarError::ChatConfiguration))
                .expect("saved provider configuration must take precedence")
                .expect("saved provider configuration");

        assert_eq!(selected.provider, OsString::from("ollama"));
        assert_eq!(
            selected.version,
            OsString::from("saved-provider-configuration")
        );
    }
    ...
    fn concurrent_manual_restart_preparation_claims_one_generation() {
        ...
        for _ in 0..2 {
            ...
            workers.push(thread::spawn(move || {
                ...
                let preparation = {
                    let mut state = supervisor.lock();
                    prepare_manual_restart_state(&mut state, "user requested retry", false)
                };
    ...
    #[test]
    fn configuration_restart_replaces_an_in_progress_generation() {
        let mut state = SupervisorState {
            status: SidecarStatus::Starting,
            phase: Some(SidecarPhase::Migrating),
            active_generation: Some(9),
        ...
        let preparation =
            prepare_manual_restart_state(&mut state, "provider configuration changed", true);

        assert!(matches!(preparation, ManualRestartPreparation::Begin(_)));
        assert_eq!(state.status, SidecarStatus::Restarting);
        assert_eq!(state.active_generation, None);
        assert!(state.endpoint.is_none());
        assert!(!state.launch_in_progress);
    ...
    #[test]
    fn configuration_restart_exposes_only_the_new_health_confirmed_generation() {
        let root = tempfile::tempdir().expect("sidecar supervisor root");
        let supervisor = SidecarSupervisor::new(
            root.path().join(DATABASE_FILE_NAME),
            root.path().join("cache"),
        );
        {
            let mut state = supervisor.lock();
            state.status = SidecarStatus::Ready;
            state.active_generation = Some(9);
        ...
        let invalidated = supervisor.connection_view();
        assert!(!invalidated.available);
        assert!(invalidated.token.is_none());
        ...
        assert!(supervisor.register_endpoint(
            ...
            Endpoint {
                port: 43126,
                token: "new-generation-token".to_owned(),
            }
        ));
        assert!(supervisor.register_phase(10, SidecarPhase::Migrating));
        assert!(supervisor.register_phase(10, SidecarPhase::Recovering));
        assert!(supervisor.register_phase(10, SidecarPhase::StartingServer));
        assert!(supervisor.register_ready_signal(10));
        assert!(!supervisor.connection_view().available);
        supervisor.mark_ready(9);
        assert!(!supervisor.connection_view().available);

        supervisor.mark_ready(10);
        let ready = supervisor.connection_view();
        assert!(ready.available);
        assert_eq!(ready.port, Some(43126));
        assert_eq!(ready.token.as_deref(), Some("new-generation-token"));
        let diagnostics = supervisor.diagnostics_view();
        assert_eq!(diagnostics.generation, Some(10));
        assert_eq!(
            diagnostics.last_restart_reason.as_deref(),
            Some("provider configuration changed")
        );
    }
    ...
    fn lifecycle_gate_blocks_launch_until_the_old_process_group_exits() {
        ...
        let (_receiver, process) =
            spawn_managed_process(command, "test-token", None).expect("managed process");
    ...
    fn command_arguments_are_fixed_and_preserve_paths_with_spaces() {
        ...
        assert_eq!(arguments[0], "--sidecar-secrets-stdin");
        assert!(!arguments.iter().any(|argument| argument == "--token"));
    }

    #[test]
    fn command_arguments_include_only_the_secret_frame_flag_when_a_key_is_configured() {
        let arguments = SidecarArguments {
            database_path: PathBuf::from("/tmp/learning-core.sqlite3"),
            documents_path: PathBuf::from("/tmp/documents"),
            seed_demo: false,
            embedding: None,
            chat: Some(ChatArguments {
                provider: OsString::from("openai-compatible"),
                base_url: OsString::from("https://api.example.com/v1"),
                model: OsString::from("model"),
                version: OsString::from("model"),
                api_key: Some(OsString::from("test-key")),
            }),
        }
        .into_os_strings();
        assert!(arguments
            .iter()
            .any(|argument| argument == "--chat-api-key-frame"));
        assert!(!arguments.iter().any(|argument| argument == "test-key"));
    }
