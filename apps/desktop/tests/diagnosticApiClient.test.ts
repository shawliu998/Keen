...
describe("diagnostic learning-core contract", () => {
  ...
  it("rejects forged diagnostic state/status pairs, except paused recovery states", () => {
    ...
    expect(diagnosticReadResponseSchema.safeParse({ ...notStarted, session: { ...session("paused"), resume_from_status: "goal_confirmation" } }).success).toBe(true);
    expect(diagnosticReadResponseSchema.safeParse({ ...pending, session: { ...session("paused", 1), resume_from_status: "diagnosing" } }).success).toBe(true);
    expect(diagnosticReadResponseSchema.safeParse({ ...answered, session: { ...session("paused", 2), resume_from_status: "studying" } }).success).toBe(true);
    expect(diagnosticReadResponseSchema.safeParse({ ...pending, session: { ...session("paused", 1), resume_from_status: "studying" } }).success).toBe(false);
  });
