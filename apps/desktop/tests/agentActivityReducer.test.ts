...
describe("Agent activity reducer", () => {
  const completionArtifact = {
    ...
    playbookSlug: "source-grounded-learning",
    playbookVersion: 1,
    playbookDefinitionHash: "b".repeat(64),
    whyNow: "A focused explanation will help.",
    summary: "Differentiate the outside, then multiply by the derivative inside.",
    explanationMarkdown: "# Private full explanation\nThis stays in the learning document.",
    sources: [{
      sourceHandle: "source-1",
      documentId: "document-1",
      documentName: "Chain rule note.md",
      pageNumber: 1,
      sectionPath: ["Derivatives", "Chain rule"],
      metadata: { internal: "must not project" },
      chunkId: "chunk-1",
    ...
    whatNext: {
      label: "Continue to targeted practice",
      action: "continue_practice",
      practiceRunId: "practice-1",
      practicePrompt: "Private prompt.",
      sessionRevision: 2,
    },
  };
  ...
  it("strictly projects only the learner-facing completion record", () => {
    const state = reduce([{
      id: "completion-1",
      type: "checkpoint",
      data: { label: "completion_published", data: completionArtifact },
    }]);

    expect(state.learningRecord).toEqual({
      summary: "Differentiate the outside, then multiply by the derivative inside.",
      sources: [{ documentName: "Chain rule note.md", pageNumber: 1, sectionPath: ["Derivatives", "Chain rule"] }],
      whatNext: "Continue to targeted practice",
    });
    expect(JSON.stringify(state.learningRecord)).not.toContain("Private");
    expect(JSON.stringify(state.learningRecord)).not.toContain("metadata");
    expect(JSON.stringify(state.learningRecord)).not.toContain("chunkContentHash");
  });

  it("deduplicates completion events, prefers the latest valid record, and clears it on reset", () => {
    const first: AgentRunEvent = {
      id: "completion-first",
      type: "checkpoint",
      data: { label: "completion_published", data: completionArtifact },
    };
    const firstState = reduce([first, first]);
    expect(firstState.receivedEventIds).toEqual(["completion-first"]);

    const latestState = agentActivityReducer(firstState, {
      type: "event",
      event: {
        ...
        data: {
          ...
          data: { ...completionArtifact, artifactId: "artifact-2", summary: "Use the latest verified explanation." },
        },
      },
    });
    expect(latestState.learningRecord?.summary).toBe("Use the latest verified explanation.");
    expect(agentActivityReducer(latestState, { type: "reset" }).learningRecord).toBeNull();
  });

  it("ignores malformed or unrelated checkpoints without suppressing normal content", () => {
    const state = reduce([
      { id: "content-1", type: "content_delta", data: { delta: "Normal conversation" } },
      { id: "completion-invalid", type: "checkpoint", data: { label: "completion_published", data: { summary: "missing durable contract" } } },
      { id: "other-checkpoint", type: "checkpoint", data: { label: "other", data: { summary: "also ignored" } } },
    ]);

    expect(state.content).toBe("Normal conversation");
    expect(state.learningRecord).toBeNull();
  });
