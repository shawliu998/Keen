...
function advanced(outcome: "applied" | "replayed" = "applied") { return { ...answered(outcome), session: session("studying", successor.id), plan: advancedPlan, current_unit: successor }; }
...
describe("Targeted Practice learning-core client", () => {
  ...
  it("accepts an applied answer that advances to a truthful successor and rejects a forged studying state", async () => {
    const accepted = createLearningCoreClient("http://127.0.0.1:8080", token, vi.fn(async () => json(advanced())) as unknown as typeof fetch);
    await expect(accepted.answerStudySessionPractice("session-1", "run-1", { course_id: "course-1", expected_revision: 3, idempotency_key: "practice-answer-001", response: "chain rule" })).resolves.toMatchObject({ session: { status: "studying", current_unit_id: "unit-2" } });
    const forged = createLearningCoreClient("http://127.0.0.1:8080", token, vi.fn(async () => json({ ...advanced(), plan, current_unit: unit, session: session("studying") })) as unknown as typeof fetch);
    await expect(forged.answerStudySessionPractice("session-1", "run-1", { course_id: "course-1", expected_revision: 3, idempotency_key: "practice-answer-001", response: "chain rule" })).rejects.toBeInstanceOf(LearningCoreSchemaError);
  });
});
