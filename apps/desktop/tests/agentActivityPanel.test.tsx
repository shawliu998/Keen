...
describe("AgentActivityPanel", () => {
  ...
  it("shows a bounded learning record instead of raw model JSON after completion", () => {
    renderPanel({
      runId: "run-1",
      state: state({
        status: "completed",
        ...
        terminal: true,
        content: '{"explanationMarkdown":"must not render","metadata":{"secret":"must not render"}}',
        learningRecord: {
          summary: "Use the chain rule by differentiating the outer function, then the inner function.",
          sources: [{ documentName: "Chain rule study note.md", pageNumber: 1, sectionPath: ["Derivatives", "Chain rule"] }],
          whatNext: "Continue to targeted practice",
        },
      }),
    });

    const record = screen.getByRole("heading", { name: "Learning record" }).closest("section");
    expect(record).not.toBeNull();
    expect(within(record as HTMLElement).getByText(/differentiat/i)).toBeInTheDocument();
    expect(within(record as HTMLElement).getByText("Chain rule study note.md · p. 1 · Derivatives › Chain rule")).toBeInTheDocument();
    expect(within(record as HTMLElement).getByText("Continue to targeted practice")).toBeInTheDocument();
    expect(screen.queryByLabelText("Agent response")).not.toBeInTheDocument();
    expect(screen.queryByText(/explanationMarkdown|must not render|secret/)).not.toBeInTheDocument();
  });
  ...
  it("allows cancellation only for active runs and omits unavailable actions", async () => {
    const user = userEvent.setup();
    ...
    view.rerender(<AgentActivityPanel runId="run-1" state={state({ status: "running", durableStatus: "running" })} cancelPending onCancel={onCancel} />);
