...
export function OpeningDiagnostic({
  ...
  const restore = useCallback(async (expectedScope: string): Promise<DiagnosticReadResponse | null> => {
    ...
    try {
      ...
      if (!isScopeCurrent(scopeRef.current, expectedScope, mountedRef.current)) return null;
      setLoading(false);
      if (abortError(error)) setNotice({ tone: "cancelled", text: "Diagnostic restore was cancelled. No new write was sent." });
      else setNotice({ tone: "error", text: "Keen could not restore this local diagnostic. Retry after the learning core recovers." });
      return null;
    } finally {
  ...
  const run = async (kind: Operation) => {
    ...
    try {
      ...
      if (!isScopeCurrent(scopeRef.current, expectedScope, mountedRef.current)) return;
      if (abortError(error)) {
        setNotice({ tone: "cancelled", text: "Request cancelled. Keen will restore the local diagnostic before another write." });
        ...
        setRetryIntent(null);
        if (restored) {
          setNotice({ tone: "error", text: "This session changed before the action completed. Keen restored the latest local diagnostic." });
        } else {
          setDiagnostic(null);
          setNotice({ tone: "error", text: "This session changed before the action completed, but Keen could not restore the latest local diagnostic. Retry after the learning core recovers; no write was retried." });
        }
      } else {
        const restored = await reconcile(expectedScope);
        if (wasApplied(kind, restored)) setRetryIntent(null);
