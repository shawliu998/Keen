...
export function ActiveRecall({
  client, sessionId, courseId, currentUnitId, stateScope, session, paused, onSessionChanged, onOutcomeChange,
}: {
  ...
  useLayoutEffect(() => {
    if (outcome === "answered") resultHeadingRef.current?.focus({ preventScroll: true });
  }, [outcome]);
  useLayoutEffect(() => {
    if (outcome === "pending" && !paused) responseRef.current?.focus({ preventScroll: true });
  }, [outcome, paused]);
  if (loading) return <LearningStepRestore label="Restoring active recall…" />;
  ...
  if (paused) return <Card className="checkpoint"><Badge tone="warning">Session paused</Badge><h3>Active recall restored</h3><p>No active-recall action was sent while this session is paused. Resume this local session before continuing.</p></Card>;
  if (outcome === "answered" && recall?.grade) return <Card className="checkpoint learning-outcome recall-result" aria-labelledby="active-recall-title">
    <Badge tone={recall.grade.correct ? "success" : "warning"}>Recall recorded</Badge>
    <h2 id="active-recall-title" ref={resultHeadingRef} tabIndex={-1}>{recall.grade.correct ? "Recalled correctly" : "Review needed"}</h2>
    <p className="learning-outcome-copy">{recall.grade.correct
      ? "You recalled the key idea. Continue with the saved next step below."
      : "Your response is saved. Keen will use it to choose the next explanation or practice step."}</p>
    <dl className="learning-outcome-metrics">
      <div><dt>Score</dt><dd>{recall.grade.score} / {recall.grade.max_score}</dd></div>
      <div><dt>Mastery evidence</dt><dd>Recorded</dd></div>
    </dl>
  </Card>;
  ...
  if (outcome === "pending" && recall?.checkpoint) return <Card className="checkpoint" aria-labelledby="active-recall-title">
    <h3 id="active-recall-title">{recall.checkpoint.prompt}</h3><p className="recall-input-guidance">Use plain language. Mathematical notation is optional, so you can describe the idea without formatting a formula.</p><label className="diagnostic-response-label" htmlFor="active-recall-response">Your response</label>
    <textarea ref={responseRef} id="active-recall-response" maxLength={8000} value={retry?.kind === "answer" ? retry.response : response} disabled={operation !== null || retry?.kind === "answer"} onChange={(event) => setResponse(event.target.value)} placeholder="Recall the answer in your own words." />
