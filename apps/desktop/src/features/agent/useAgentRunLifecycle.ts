...
export type AgentMutationActionState = {
  target: { action: "undo" | "redo"; mutationId: string } | null;
...
export type AgentRuntimeContextValue = {
  ...
  setActivityContext: (context: AgentActivityContext) => void;
  /** Re-read the selected workspace after an out-of-band Agent run changes it. */
  refreshActivityContext?: (knownRunId?: string) => void;
  confirmApproval?: (approvalId: string) => Promise<boolean>;
  rejectApproval?: (approvalId: string) => Promise<boolean>;
};

export type AgentActivityContext = AgentRunActivityContext | null;

export function isSameAgentActivityContext(a: AgentActivityContext, b: AgentActivityContext): boolean {
  if (a === b) return true;
  if (a === null || b === null) return false;
  if ("conversationId" in a && "conversationId" in b) return a.conversationId === b.conversationId;
  if ("studySessionId" in a && "studySessionId" in b) return a.studySessionId === b.studySessionId;
  return false;
}
...
export function activityUnavailableIssue(): AgentRuntimeIssue {
  return {
    code: "activity_unavailable",
    message: "Keen confirmed a newer Agent run, but its recorded activity is not readable yet. An older run is not being shown as current.",
    retryable: true,
    recovery: "Retry the activity refresh after the local activity index catches up.",
    automaticRecovery: false,
  };
}

export function mutationIssue(
  error?: LearningCoreResponseError,
  action?: "undo" | "redo",
): AgentRuntimeIssue {
  const code = error?.detail?.code;
  const actionLabel = action === "undo" ? "Undo" : action === "redo" ? "Redo" : "Undo or Redo";
  if (code === "mutation_not_found") {
    return {
      code: "run_failed",
      message: `The selected mutation is no longer available, so the ${actionLabel} change was not made.`,
      retryable: false,
  ...
  return {
    code: "run_failed",
    message: `Keen could not confirm the requested ${actionLabel} action, so no result is shown.`,
