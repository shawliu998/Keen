import { learningInterventionArtifactSchema, type AgentRunEvent, type AgentRunStatus } from "@keen/api-client";
import type {
  AgentActivityAction,
  AgentActivityState,
  AgentActivityStatus,
...
function learningRecordFromCheckpoint(data: unknown): AgentLearningRecord | null {
  const parsed = learningInterventionArtifactSchema.safeParse(data);
  ...
  return {
    ...
    whatNext: parsed.data.whatNext.label,
  };
}
...
export function agentActivityReducer(state: AgentActivityState, action: AgentActivityAction): AgentActivityState {
  ...
  switch (event.type) {
    ...
    case "checkpoint":
      ...
      if (event.data.label === "completion_published") {
        const learningRecord = learningRecordFromCheckpoint(event.data.data);
        return learningRecord ? { ...next, learningRecord } : next;
      }
      return next;
    case "metadata":
    case "status":
