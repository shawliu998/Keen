...
export function ConversationPage() {
  ...
  return (
    <div className="conversation-page">
      ...
      <div className="messages">{messages.map((message) => <article className={`message ${message.role}`} key={message.id}>
        <div className="avatar">{message.role === "assistant" ? "K" : "AC"}</div><div className="message-body"><strong>{message.role === "assistant" ? "Keen" : "You"}</strong>
          ...
          {message.role === "assistant" && <><button className="citation" onClick={() => setInspector({ eyebrow: "Unverified demo citation", title: "Linear Algebra — Chapter 5", body: "This excerpt is deterministic seed content. No source PDF is present, so it is not a validated citation.", meta: ["Nominal page 14", "Demo chunk d1-p14-c3", "Verification unavailable"] })}><BookOpenText size={13} />Demo citation · p. 14</button>
            <div className="message-actions"><button aria-label="Copy unavailable" disabled><Copy size={13} /></button><button aria-label="Regenerate unavailable" disabled><RefreshCw size={13} /></button><button aria-label="Edit and resend unavailable" disabled><Edit3 size={13} /></button><button aria-label="Branch unavailable" disabled><GitBranch size={13} /></button></div></>}
        </div></article>)}
      ...
      <div className="conversation-composer"><div className="composer card"><textarea aria-label="Reply" value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Ask a demo follow-up…" /><div className="composer-bottom"><div className="composer-tools"><IconButton label="Attachment unavailable" disabled><Paperclip size={16} /></IconButton><span className="draft-state">Draft saved locally · demo only</span></div><button className="send-button" aria-label={generating ? "Stop demo response" : "Send demo message"} onClick={generating ? () => setGenerating(false) : send}>{generating ? <Square size={14} /> : <Send size={15} />}</button></div></div></div>
    </div>
  );
}
