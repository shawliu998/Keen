...
export function PlannerPage() {
  ...
  return (
    <Page title="Study Planner" description="A deterministic demo schedule. Syllabus parsing and calendar writes are not connected." actions={<><Button disabled><FileUp size={14} />Import unavailable</Button><Button className="primary" onClick={() => setConfirmCalendar(true)}><Calendar size={14} />Preview calendar confirmation</Button></>}>
      ...
      <div className="calendar-toolbar"><Button><ChevronLeft size={14} /></Button><strong>July 13–19, 2026</strong><Button><ChevronRight size={14} /></Button><span /><Badge tone="accent">Adaptive plan</Badge></div>
      <div className="week-grid">{days.map((day, dayIndex) => <div className={`day-column ${dayIndex === 2 ? "today" : ""}`} key={day}><header><span>{day.split(" ")[0]}</span><strong>{day.split(" ")[1]}</strong>{dayIndex === 2 && <i>Today</i>}</header><div className="day-body">{tasks.filter((t) => t.day === dayIndex).map((task) => <Card className={`plan-task ${task.done ? "done" : ""}`} key={task.id}><div><GripVertical size={12} /><Badge>{task.course}</Badge></div><strong>{task.title}</strong><span><Clock3 size={11} />{task.time} · {task.duration}m</span><button onClick={() => toggle(task.id)} aria-label={task.done ? "Mark incomplete" : "Mark complete"}>{task.done ? <Check size={13} /> : "Done"}</button></Card>)}{!tasks.some((t) => t.day === dayIndex) && <button className="add-session">+ Add session</button>}</div></div>)}</div>
      <div className="planner-note"><WandSparkles size={15} /><p><strong>Why this order?</strong> Eigenvectors is scheduled first because it is overdue, low-mastery, and required for your exam. Biology is due under spaced repetition today.</p><Button>Review rationale</Button></div>
