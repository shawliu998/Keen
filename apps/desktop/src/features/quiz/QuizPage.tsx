...
export function QuizPage() {
  ...
  if (finish) return <Page title="Practice complete" description="Demo result only: this session does not mutate mastery or schedule a review."><Card className="quiz-summary"><div className="score-ring">{score}/{questions.length}</div><h2>{score === questions.length ? "Excellent recall" : "Good diagnostic signal"}</h2><p>This result remains in the current UI session until the learning-core assessment contract is connected.</p><div className="summary-stats"><span><small>Accuracy</small><strong>{Math.round(score / questions.length * 100)}%</strong></span><span><small>Mastery change</small><strong>Not applied</strong></span><span><small>Next review</small><strong>Not scheduled</strong></span></div><Button className="primary" onClick={() => { setIndex(0); setScore(0); }}>Practice again<RotateCcw size={14} /></Button></Card></Page>;
  return (
    <Page title="Eigenvectors quick check" description="3 questions · Adaptive practice" actions={<Button>Exit</Button>}>
