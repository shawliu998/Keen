...
type BrowserVisualFixture = {
  status: LearningCoreStatus;
  demoState?: DemoState;
  demoStateError?: Error;
};

function browserVisualFixture(desktop: boolean): BrowserVisualFixture | null {
  if (!import.meta.env.DEV || desktop || typeof window === "undefined") return null;
  const params = new URLSearchParams(window.location.search);
  if (params.get("visualTest") !== "true") return null;
  switch (params.get("visualCoreState")) {
    case "unavailable":
      return { status: "unavailable" };
    case "loading":
      return { status: "health_checking" };
    case "empty":
      return { status: "healthy", demoState: { courses: [], tasks: [], mastery: [] } };
    case "error":
      return { status: "healthy", demoStateError: new Error("Deterministic visual fixture: invalid local learning data.") };
    default:
      return null;
  }
}
...
export function LearningCoreProvider({ children }: { children: ReactNode }) {
  ...
  const visualFixture = useMemo(() => browserVisualFixture(desktop), [desktop]);
  ...
  const status: LearningCoreStatus = (() => {
    if (visualFixture) return visualFixture.status;
  ...
  const value = useMemo<LearningCoreContextValue>(() => ({
    ...
    demoState: visualFixture ? visualFixture.demoState : demoStateQuery.data,
    demoStatePending: visualFixture ? false : status === "healthy" && demoStateQuery.isPending,
    demoStateError: visualFixture ? visualFixture.demoStateError ?? null : toError(demoStateQuery.error),
    visualFixture: visualFixture !== null,
    retry,
  }), [client, connectionGeneration, demoStateQuery.data, demoStateQuery.error, demoStateQuery.isPending, errorKind, retry, retryError, serviceMessage, status, visualFixture]);
