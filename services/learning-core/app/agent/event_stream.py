...
class AgentEventStore:
    ...
    def get_latest_run_activity(
        self,
        *,
        conversation_id: str | None = None,
        study_session_id: str | None = None,
    ) -> tuple[dict | None, list[DurableAgentEvent]]:
        """Load a restart-safe public run snapshot for one learning context."""

        if conversation_id is not None:
            self._validate_identifier(conversation_id, label="conversation ID")
        if study_session_id is not None:
            self._validate_identifier(study_session_id, label="study session ID")
        with self._database.connection() as connection:
            repository = AgentRepository(connection)
            # sqlite3's legacy transaction handling does not start a transaction
            # for SELECT statements. Explicitly establish a read transaction so
            # selecting the latest run, loading its record, and replaying its
            # events all observe one WAL snapshot.
            connection.execute("BEGIN DEFERRED")
            try:
                run = repository.find_latest_run(
                    conversation_id=conversation_id,
                    study_session_id=study_session_id,
                )
                if run is None:
                    return None, []
                # Convert every stored row back through the durable public-event
                # contract. Invalid or private persisted payloads fail closed here
                # rather than being exposed by the recovery JSON endpoint.
                events = [self._event(row) for row in repository.list_events(run["id"])]
            finally:
                connection.rollback()
        return run, events
    ...
    def _event(row: dict) -> DurableAgentEvent:
        payload = row["payload"]
        if row["event_type"] == "checkpoint" and not _is_checkpoint_envelope(payload):
            # Historical profile completions were persisted as their raw artifact.
            # Keep the database immutable, but present all public replay surfaces
            # with the checkpoint envelope required by the API contract.
            payload = {"label": "completion_published", "data": payload}
...
def _is_checkpoint_envelope(payload: object) -> bool:
    """Whether a persisted checkpoint already has the strict public shape."""

    return (
        isinstance(payload, dict)
        and set(payload) == {"label", "data"}
        and isinstance(payload.get("label"), str)
        and 0 < len(payload["label"]) <= 200
