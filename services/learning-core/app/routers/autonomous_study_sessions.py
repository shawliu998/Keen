...
from ..schemas import (
    AdaptiveActionCompleteRequest,
    AdaptiveActionCompleteResponse,
    AdaptiveActionResponse,
    AdaptiveStudyStateResponse,
    ...
    StudySessionControlRequest,
    StudySessionControlResponse,
...
from ..services.adaptive_study_session import (
...
from ..services.study_session_control import (
    StudySessionCommand,
    StudySessionControlConflictError,
    StudySessionControlNotFoundError,
    StudySessionControlResult,
    StudySessionControlService,
)
...
def _study_session_control_conflict() -> HTTPException:
    return HTTPException(
        status_code=409,
        detail={
            "code": "study_session_control_conflict",
            "message": "The study session changed before this action could be applied.",
            "retryable": True,
            "recoveryAction": "Refresh the session, then use its current pause or resume action.",
            "automaticRecovery": False,
        },
    )


def _study_session_control_service_error() -> HTTPException:
    return HTTPException(
        status_code=503,
        detail={
            "code": "study_session_control_temporarily_unavailable",
            "message": "Keen could not safely determine whether the session action was saved.",
            "retryable": True,
            "recoveryAction": "Refresh the session. If its state is unchanged, retry with the same action.",
            "automaticRecovery": False,
            "outcomeMayBeDurable": True,
        },
    )
...
def _adaptive_not_found() -> HTTPException:
    return HTTPException(
        status_code=404,
        detail={
            "code": "study_adaptive_action_not_found",
            "message": "No adaptive Study Session action is available in the selected course.",
            "retryable": False,
            "recoveryAction": "Refresh the study session and continue its current step.",
            "automaticRecovery": False,
        },
...
def _adaptive_conflict() -> HTTPException:
    return HTTPException(
        status_code=409,
        detail={
            "code": "study_adaptive_action_conflict",
            "message": "The adaptive Study Session action changed before this request could be applied.",
            "retryable": True,
            "recoveryAction": "Refresh adaptive state, then retry using its current action revision.",
            "automaticRecovery": False,
        },
    )


def _adaptive_write_service_error() -> HTTPException:
    return HTTPException(
        status_code=503,
        detail={
            "code": "study_adaptive_action_temporarily_unavailable",
            "message": "Keen could not safely determine whether the adaptive action was saved.",
            "retryable": True,
            "recoveryAction": "Refresh adaptive state, then retry with the same idempotency key.",
            "automaticRecovery": False,
            "outcomeMayBeDurable": True,
        },
    )
...
def _adaptive_read_service_error() -> HTTPException:
    return HTTPException(
        status_code=503,
        detail={
            "code": "study_adaptive_action_temporarily_unavailable",
            "message": "Keen could not safely restore the adaptive Study Session state.",
            "retryable": True,
            "recoveryAction": "Retry. If the problem continues, return to the learning feed.",
            "automaticRecovery": False,
        },
    ...
    return AutonomousStudySessionResponse.model_validate(
        {
            ...
            "resume_from_status": session.get("resume_from_status"),
...
def _study_session_control_response(
    result: StudySessionControlResult,
    *,
    ...
    if (
        session is None
        or result.course_id != expected_course_id
        or session.course_id != expected_course_id
        or session.id != expected_session_id
    ):
        raise ValueError("study session control result is outside request scope")
    if result.command == "pause" and result.outcome == "applied":
        if session.status != "paused" or session.resume_from_status is None:
            raise ValueError("pause result did not persist a resume state")
    if result.command == "resume" and result.outcome == "applied":
        if session.status == "paused" or session.resume_from_status is not None:
            raise ValueError("resume result remained paused")
    return StudySessionControlResponse(
...
def _adaptive_action(value: dict[str, Any] | None) -> AdaptiveActionResponse | None:
    if value is None:
        return None
    return AdaptiveActionResponse.model_validate(
        {
            **value,
            "created_at": _stored_utc_datetime(value["created_at"]),
            "started_at": _stored_utc_datetime(value["started_at"], optional=True),
            "completed_at": _stored_utc_datetime(value["completed_at"], optional=True),
            "cancelled_at": _stored_utc_datetime(value["cancelled_at"], optional=True),
        }
    )


def _adaptive_state_response(
    result: AdaptiveStudyStateResult,
    *,
    expected_course_id: str,
    expected_session_id: str,
) -> AdaptiveStudyStateResponse:
    ...
    action = _adaptive_action(result.action)
    if (
        session is None
        or result.course_id != expected_course_id
        or session.course_id != expected_course_id
        or session.id != expected_session_id
        or (plan is not None and plan.session_id != expected_session_id)
    ):
        raise ValueError("adaptive study state is outside request scope")
    current_unit = None
    if result.current_unit is not None:
        current_unit = AutonomousStudySessionUnitResponse.model_validate(
            {
                key: result.current_unit[key]
                for key in (
                    "id",
                    "ordinal",
                    "concept_id",
    ...
    if result.current_unit_id != (
        current_unit.id if current_unit is not None else None
    ):
        raise ValueError("adaptive current unit pointer is inconsistent")
    if action is not None and (
        action.course_id != expected_course_id
        or action.session_id != expected_session_id
        or action.unit_id != result.current_unit_id
    ):
        raise ValueError("adaptive action is outside the current unit")
    if result.state == "action_required" and action is None:
        raise ValueError("adaptive action-required state has no action")
    if result.state in {"legacy_canonical", "canonical", "terminal"} and action:
        raise ValueError("adaptive non-action state exposed an action")
    return AdaptiveStudyStateResponse(
        course_id=result.course_id,
        session=session,
        plan=plan,
...
def _adaptive_completion_response(
    result: AdaptiveActionCompletionResult,
    *,
    expected_course_id: str,
    expected_session_id: str,
    ...
    session = _session(result.session)
    plan = _plan(result.plan)
    completed = _adaptive_action(result.completed_action)
    current = _adaptive_action(result.current_action)
    if (
        session is None
        or plan is None
        ...
        or session.course_id != expected_course_id
        or session.id != expected_session_id
        or plan.session_id != expected_session_id
        or completed.id != expected_action_id
        or completed.course_id != expected_course_id
        or completed.session_id != expected_session_id
        or completed.kind != "remediate"
        or completed.status != "completed"
        or current.course_id != expected_course_id
        or current.session_id != expected_session_id
        or current.kind != "practice"
        or current.unit_id != completed.unit_id
    ):
        raise ValueError("adaptive action completion is outside request scope")
    return AdaptiveActionCompleteResponse(
        outcome=result.outcome,
        course_id=result.course_id,
        session=session,
        plan=plan,
...
def list_study_sessions(
    ...
    try:
        with request.app.state.database.connection() as connection:
            rows = StudyRepository(connection).list_for_course(course_id, limit=limit)
        sessions = [_session(row) for row in rows]
        if any(
            session is None or session.course_id != course_id for session in sessions
        ):
            raise ValueError("study session history is outside requested course")
...
def _control_study_session(
    *,
    command: StudySessionCommand,
    payload: StudySessionControlRequest,
    request: Request,
    session_id: str,
) -> StudySessionControlResponse:
    try:
        with request.app.state.database.connection() as connection:
            result = StudySessionControlService(connection).apply(
                command=command,
        ...
        return _study_session_control_response(
            result,
            expected_course_id=payload.course_id,
            expected_session_id=session_id,
        )
    except StudySessionControlNotFoundError:
        raise _read_not_found() from None
    except StudySessionControlConflictError:
        raise _study_session_control_conflict() from None
    except (sqlite3.Error, RuntimeError, ValueError, LookupError, KeyError, TypeError):
        raise _study_session_control_service_error() from None


@router.post(
    "/study-sessions/{session_id}/pause",
    response_model=StudySessionControlResponse,
)
...
@router.post(
    "/study-sessions/{session_id}/resume",
    response_model=StudySessionControlResponse,
)
...
@router.get(
    "/study-sessions/{session_id}/adaptive-state",
    response_model=AdaptiveStudyStateResponse,
)
def get_study_adaptive_state(
    request: Request,
    session_id: str = Path(
    ...
    try:
        with request.app.state.database.connection() as connection:
            result = AdaptiveStudySessionService(connection).get(
                course_id=course_id, session_id=session_id
            )
        return _adaptive_state_response(
            result, expected_course_id=course_id, expected_session_id=session_id
        )
    except AdaptiveStudyNotFoundError:
        raise _adaptive_not_found() from None
    except (sqlite3.Error, RuntimeError, ValueError, LookupError, KeyError, TypeError):
        raise _adaptive_read_service_error() from None


@router.post(
    "/study-sessions/{session_id}/adaptive-actions/{action_id}/complete",
    response_model=AdaptiveActionCompleteResponse,
)
def complete_study_adaptive_action(
    payload: AdaptiveActionCompleteRequest,
    request: Request,
    session_id: str = Path(
        min_length=1, max_length=128, pattern=r"^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$"
    ),
    ...
    try:
        with request.app.state.database.connection() as connection:
            result = AdaptiveStudySessionService(connection).complete_remediation(
                course_id=payload.course_id,
                session_id=session_id,
                action_id=action_id,
                expected_action_revision=payload.expected_action_revision,
                idempotency_key=payload.idempotency_key,
                now=datetime.now(UTC),
            )
        return _adaptive_completion_response(
            result,
            expected_course_id=payload.course_id,
            expected_session_id=session_id,
        ...
        raise _adaptive_not_found() from None
    except AdaptiveStudyConflictError:
        raise _adaptive_conflict() from None
    except (sqlite3.Error, RuntimeError, ValueError, LookupError, KeyError, TypeError):
        raise _adaptive_write_service_error() from None
