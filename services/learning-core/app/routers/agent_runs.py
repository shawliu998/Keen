...
from ..agent.event_stream import DurableAgentEvent, encode_sse
...
class AgentRunActivityEventResponse(ApiModel):
    """One persisted, public Agent event in durable sequence order."""
    ...
    @classmethod
    def validate_public_data(cls, value: dict[str, object]) -> dict[str, object]:
        validate_bounded_json_object(value)
        _reject_hidden_reasoning(value)
        return value


class AgentRunActivityResponse(ApiModel):
    """A truthful empty-or-latest recovery snapshot for one context."""

    run: AgentRunResponse | None
    events: list[AgentRunActivityEventResponse]
...
def _activity_event_response(event: DurableAgentEvent) -> AgentRunActivityEventResponse:
    return AgentRunActivityEventResponse(
        id=event.id,
...
@router.get("/latest", response_model=AgentRunActivityResponse)
def get_latest_agent_run_activity(
    request: Request,
    ...
    """Return the last persisted run and safe events for one current context.

    This does not resume execution. A missing context match is a successful,
    explicit empty result so callers never invent activity after a refresh.
    """

    if (conversation_id is None) == (study_session_id is None):
        raise HTTPException(
            status_code=422,
            detail="provide exactly one of conversationId or studySessionId",
        )
    try:
        run, events = _manager(request).get_latest_run_activity(
            conversation_id=conversation_id,
            study_session_id=study_session_id,
        )
    except ValueError as error:
        raise HTTPException(
            status_code=422, detail="invalid Agent activity context"
        ) from error
    if run is None:
        return AgentRunActivityResponse(run=None, events=[])
    return AgentRunActivityResponse(
        run=_run_response(run),
        events=[_activity_event_response(event) for event in events],
    )
