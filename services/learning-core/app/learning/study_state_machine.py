...
# A completed unit may hand the same session back to source study for the next
# planned unit.  The final unit still follows the canonical practice -> summary
# path, so this is the only deliberate loop in the state machine.
_DIRECT_TRANSITIONS[DeepLearnState.PRACTICING] = frozenset(
    {DeepLearnState.STUDYING, DeepLearnState.SUMMARIZING}
)
