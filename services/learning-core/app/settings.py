...
class LocalChatSettings:
    """Complete chat configuration; credentials are excluded from object reprs."""
    ...
    def __post_init__(self) -> None:
        ...
        endpoint = validate_provider_endpoint(
            ProviderKind(self.provider), self.base_url
        )
        if (
            self.provider == "openai-compatible"
            and endpoint.url.startswith("https://")
            and (self.api_key is None or not self.api_key.strip())
        ):
            raise ValueError(
                "an API key is required for an OpenAI-compatible chat provider"
            )
        if self.api_key is not None and (
            len(self.api_key) > 4_096 or "\r" in self.api_key or "\n" in self.api_key
        ):
