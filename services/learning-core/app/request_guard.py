...
def _is_study_session_control_action(path: object) -> bool:
    if not isinstance(path, str):
        return False
    segments = path.split("/")
    return (
        ...
        and segments[1:3] == ["v1", "study-sessions"]
        and bool(segments[3])
        and segments[4] in {"pause", "resume"}
    )


def _is_study_active_recall_action(path: object) -> bool:
    if not isinstance(path, str):
        return False
    segments = path.split("/")
    begin = (
...
def _is_conversation_action(path: object) -> bool:
    if not isinstance(path, str):
        return False
    segments = path.split("/")
    return (
        segments == ["", "v1", "conversations"]
        or (
            len(segments) == 6
            and segments[1:3] == ["v1", "conversations"]
            and bool(segments[3])
            and segments[4:6] == ["answers", "stream"]
        )
        or (
            len(segments) == 7
            and segments[1:3] == ["v1", "conversations"]
...
class RequestGuardMiddleware:
    ...
    async def __call__(
        ...
        is_bounded_json = method == "POST" and (
            ...
            or _is_study_session_control_action(path)
            ...
            or _is_conversation_action(path)
        )
