...
from ..agent.profile import AgentRunProfile
...
class AgentRuntimeManager:
    ...
    def provider_configured(self) -> bool:
        return self._provider_factory is not None
    ...
    async def create_run(
        ...
        *,
        ...
        await self._settle_terminal_active_task()
        request_signature = (
            kind,
            user_intent,
            mode,
        ...
        async with self._lock:
            ...
            task = asyncio.create_task(
                self._execute(proposed_run_id, provider, run_profile),
    ...
    async def _settle_terminal_active_task(self) -> None:
        """Close the narrow gap between durable completion and slot cleanup."""
    ...
    def get_latest_run_activity(
        ...
        *,
        ...
        return self._event_store.get_latest_run_activity(
            conversation_id=conversation_id,
            study_session_id=study_session_id,
        )
    ...
    async def _execute(
        ...
        run_profile: AgentRunProfile | None,
    ) -> None:
            ...
            with self._database.connection() as audit_connection:
                ...
                if run_profile is not None:
                    if course_scope_id is None:
                        raise RuntimeError(
                            "purpose-scoped Agent run requires a course scope"
                        )
                    run_profile.register_tools(
                        registry,
                    ...
                    tool_runtime = (
                        ...
                        else ProviderToolRuntime.from_readonly_registry(
                            registry,
                            executor,
                        )
                    )
                ...
                orchestrator = AgentOrchestrator(
                    ...
                    level2_proposer=(
                        self._level2_approvals
                        if tool_runtime is not None and tool_runtime.proposal_tool_names
                        else None
                    ),
