    ...
    def complete(
        ...
        *,
        ...
        with write_scope(self.connection, commit=True):
            ...
            if (
                StudyRepository(self.connection).get_locked_successor(
                    session_id=session_id, unit_id=str(lineage["unit_id"])
                )
                is not None
            ):
                raise StudySummaryConflictError(
                    "study summary is reserved for the final planned unit"
                )
            if int(lineage["remaining_units"]) != 0:
                raise StudySummaryConflictError(
                    "study summary is reserved for the final planned unit"
                )
    ...
    def _lineage(
        ...
        rows = self.connection.execute(
            f"""SELECT r.id AS practice_run_id, r.unit_id, r.assessment_id, r.item_id, r.concept_id,
               ...
               ORDER BY CASE WHEN s.current_unit_id = r.unit_id THEN 0 ELSE 1 END,
                        u.ordinal DESC, r.created_at DESC, r.id DESC""",
        ...
        if len(rows) > 1 and rows[0]["unit_id"] == rows[1]["unit_id"]:
            raise RuntimeError("summary unit has multiple answered practice runs")
        value = dict(rows[0])
