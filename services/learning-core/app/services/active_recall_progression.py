...
class ActiveRecallProgressionService:
    ...
    def begin(
        ...
        *,
        ...
        with write_scope(self.connection, commit=True):
            ...
            try:
                replay = ActiveRecallRepository(self.connection).replay_begin(
                    course_id=course_id,
    ...
    def get(self, *, course_id: str, session_id: str) -> ActiveRecallReadResult:
        ...
        rows = self.connection.execute(
            "SELECT * FROM study_active_recall_runs WHERE session_id = ? ORDER BY created_at, id",
            (session_id,),
        ).fetchall()
        if len(rows) > 1:
            raise RuntimeError("study session has multiple active recall runs")
    ...
    def _result(
        ...
        *,
        ...
        read = self.get(course_id=course_id, session_id=session_id)
        if read.run is None or read.checkpoint is None or read.run["id"] != run["id"]:
        ...
        return ActiveRecallProgressionResult(
            ...
            run=read.run,
            grade=grade if grade is not None else read.grade,
        )
