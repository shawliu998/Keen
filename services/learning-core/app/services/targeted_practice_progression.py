...
class TargetedPracticeProgressionService:
    ...
    def begin(
        ...
        *,
        ...
        with write_scope(self.connection, commit=True):
            ...
            try:
                adaptive_action = AdaptiveStudySessionService(
                    self.connection
                ).require_pending_practice(
                    course_id=course_id,
                    session_id=session_id,
                    unit_id=str(unit["id"]),
                )
            except AdaptiveStudyConflictError as error:
                raise TargetedPracticeConflictError(str(error)) from error
            ...
            if adaptive_action is not None:
                AdaptiveStudySessionService(self.connection).complete_practice(
                    action_id=str(adaptive_action["id"]),
                    practice_run_id=str(run["id"]),
    ...
    def answer(
        ...
        *,
        ...
        with write_scope(self.connection, commit=True):
            ...
            study = StudyRepository(self.connection)
            successor = study.get_locked_successor(
                session_id=session_id, unit_id=str(unit["id"])
            )
            if successor is None:
                transition_study_state("practicing", "summarizing")
                summarizing = study.transition_session(
                    session_id,
                    status="summarizing",
                ...
                transition_study_state("practicing", "studying")
                advanced, _ = study.advance_current_unit(
                    session_id=session_id,
                    unit_id=str(unit["id"]),
                ...
                if (
                    advanced["status"] != "studying"
                    or advanced["current_unit_id"] != successor["id"]
                ):
                    raise RuntimeError("practice answer did not advance the study unit")
            return self._result(
                course_id=course_id,
                session_id=session_id,
                run=sealed,
    ...
    def get(self, *, course_id: str, session_id: str) -> TargetedPracticeReadResult:
        ...
        repository = PracticeRepository(self.connection)
        run = (
        ...
        if run is None and effective in {
            "studying",
            "checkpoint",
            "active_recall",
            "practicing",
