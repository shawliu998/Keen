...
class AutonomousRecommendationCoordinator:
    ...
    def observe_and_recommend(
        ...
        *,
        ...
        completed task is replayed inside the same learner-local day; a later day may
        ...
        # This prevents two different learner-local day-bucket keys from both creating
        ...
        with write_scope(self.connection, commit=True):
            ...
            completed = task_repository.find_completed_recommendation_for_day(
                course_id=snapshot.course_id,
                source_type=source_type,
                source_id=source_id,
                concept_id=concept_id,
                candidate_id=candidate.id,
                completed_from=_day_start(local_time).astimezone(UTC).isoformat(),
                completed_through=_day_end(local_time).astimezone(UTC).isoformat(),
            )
            ...
            bucket = local_time.date().isoformat()
            ...
            task, created = task_repository.create_task(
                ...
                due_at=_day_end(local_time).astimezone(UTC).isoformat(),
                ...
                scheduled_for=_day_start(local_time).astimezone(UTC).isoformat(),
...
def _time_zone(value: str) -> ZoneInfo:
    if not isinstance(value, str) or not value or len(value) > 100:
        raise ValueError("time_zone must be a bounded IANA time zone")
    try:
        return ZoneInfo(value)
    except ZoneInfoNotFoundError:
        raise ValueError("time_zone must name an available IANA time zone") from None
