...
class AutonomousStudySessionService:
    ...
    def start_or_resume(
        self, *, course_id: str, task_id: str, now: datetime
        ...
        with write_scope(self.connection, commit=True):
            ...
            blocked = _task_block(task, course_id=scoped_course_id)
            ...
            plan = study_repository.save_plan(
                ...
                session_id=session_id,
                version=1,
                rationale="Two bounded units cite indexed course source chunks.",
                units=units,
                created_at=current_time.isoformat(),
                commit=False,
            )
    ...
    def _source_chunks(self, *, course_id: str, concept_name: str) -> list[sqlite3.Row]:
        # Prefer chunks that contain the persisted concept label, then use a
        # deterministic indexed-course fallback.  This remains useful for CJK
        # and non-textual labels without claiming a relevance score.
        return list(
            self.connection.execute(
                ...
                ORDER BY
                    CASE WHEN instr(lower(ch.content), lower(?)) > 0 THEN 0 ELSE 1 END,
                    d.id, ch.ordinal, ch.id
                LIMIT 2
                """,
...
def _task_block(
    task: dict[str, Any] | None, *, course_id: str
