        ...
        for fused in fused_results:
            for chunk_id in fused.chunk_ids:
                ...
                row = connection.execute(
                    f"""
                    SELECT chunks.id, chunks.document_id, chunks.page_number,
...
class CitationValidator:
    ...
    def validate(
        ...
        if row is None or str(row["chunk_id"]) not in source.chunk_ids:
            return None
        try:
        ...
        *,
        ...
        yield AnswerEvent(
            "retrieval",
            {
                ...
                "chunks": [source.public_dict() for source in sources],
            },
        )
        ...
        try:
            ...
            invalid_count += invalid
            for action, value in final_actions:
                if action == "text":
                    yield AnswerEvent("delta", {"text": str(value)})
            if invalid_count:
                yield AnswerEvent(
                    "warning",
                    {
