...
class ActiveRecallRepository:
    ...
    def get_run_for_unit(
        self, *, course_id: str, session_id: str, unit_id: str
    ) -> dict[str, Any] | None:
        rows = self.connection.execute(
            """SELECT * FROM study_active_recall_runs
               WHERE course_id = ? AND session_id = ? AND unit_id = ?
               ORDER BY created_at, id""",
            (course_id, session_id, unit_id),
        ).fetchall()
        if len(rows) > 1:
            raise RuntimeError("study unit has multiple active recall runs")
    ...
    def get_latest_run_for_session(
        ...
        rows = self.connection.execute(
            """SELECT r.* FROM study_active_recall_runs r
               JOIN study_units u ON u.id = r.unit_id
               WHERE r.course_id = ? AND r.session_id = ?
               ORDER BY u.ordinal DESC, r.created_at DESC, r.id DESC""",
            (course_id, session_id),
        ).fetchall()
        if len(rows) > 1 and rows[0]["unit_id"] == rows[1]["unit_id"]:
            raise RuntimeError("latest study unit has multiple active recall runs")
        return self._public(dict(rows[0])) if rows else None
