...
class PracticeRepository:
    ...
    def get_run_for_unit(
        ...
        if len(rows) > 1:
            raise RuntimeError("study unit has multiple practice runs")
    ...
    def get_latest_run_for_session(
        ...
        rows = self.connection.execute(
            """SELECT r.* FROM study_practice_runs r
        ...
        if len(rows) > 1 and rows[0]["unit_id"] == rows[1]["unit_id"]:
            raise RuntimeError("latest study unit has multiple practice runs")
        return self._public(dict(rows[0])) if rows else None
