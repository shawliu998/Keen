...
class AgentRepository:
    ...
    def find_latest_run(
        ...
        *,
        ...
        """Return the newest persisted run for one durable learning context.

        A session created by focused study maps directly to
        ``agent_runs.study_session_id``. A blocked focused-study request has no
        session and therefore truthfully returns no Agent run.
        """

        if (conversation_id is None) == (study_session_id is None):
            raise ValueError("exactly one Agent activity context is required")
