        ...
        for ordinal in range(2):
            connection.execute(
                ...
                (
                    ...
                    ordinal,
                    f"Autonomous API concept cited source excerpt {ordinal + 1}.",
                    f"{ordinal + 1:064x}",
                    NOW,
                ),
            )
...
def _control_session(
    ...
    *,
    ...
    return client.post(
        f"/v1/study-sessions/{session_id}/{command}",
        headers=AUTH,
        json={
            "course_id": course_id,
...
def test_pause_and_resume_api_are_persisted_scoped_and_idempotent(
    client: TestClient,
) -> None:
    _course_and_concept(client)
    _indexed_source(client)
    _task(client, task_id="task-session-control")
    created = _start(client, "task-session-control").json()["session"]
    session_id = created["id"]

    with client.app.state.database.connection() as connection:
        protected_before = {
            table: connection.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0]
            for table in ("mastery_evidence", "review_items", "review_attempts")
        }

    paused_response = _control_session(
        client,
        session_id,
        "pause",
        revision=created["revision"],
        key="pause-session-control-01",
    )
    replayed_pause = _control_session(
        client,
        session_id,
        "pause",
        revision=created["revision"],
    ...
    assert paused["session"]["resume_from_status"] == "goal_confirmation"
    assert paused["session"]["revision"] == created["revision"] + 1
    assert replayed_pause.status_code == 200
    assert replayed_pause.json()["outcome"] == "replayed"
    assert replayed_pause.json()["session"] == paused["session"]

    restored = _read_session(client, session_id)
    assert restored.status_code == 200
    assert restored.json()["session"]["resume_from_status"] == "goal_confirmation"

    resumed_response = _control_session(
        client,
        session_id,
        "resume",
        revision=paused["session"]["revision"],
        key="resume-session-control-01",
    )
    replayed_resume = _control_session(
        client,
        session_id,
        "resume",
        revision=paused["session"]["revision"],
    ...
    assert resumed["session"]["status"] == "goal_confirmation"
    assert resumed["session"]["resume_from_status"] is None
    assert resumed["session"]["revision"] == paused["session"]["revision"] + 1
    assert replayed_resume.status_code == 200
    assert replayed_resume.json()["outcome"] == "replayed"
    assert replayed_resume.json()["session"] == resumed["session"]
...
def test_session_control_rejects_stale_foreign_and_reused_commands(
    client: TestClient,
) -> None:
    _course_and_concept(client)
    _indexed_source(client)
    _task(client, task_id="task-session-control-conflict")
    created = _start(client, "task-session-control-conflict").json()["session"]
    session_id = created["id"]
    pause_key = "pause-session-control-02"

    paused = _control_session(
        client,
        session_id,
        "pause",
        revision=created["revision"],
    ...
    stale = _control_session(
        client,
        session_id,
        "resume",
        revision=created["revision"],
        key="resume-session-stale-02",
    )
    reused = _control_session(
        client,
        session_id,
        "resume",
    ...
    foreign = _control_session(
        client,
        session_id,
        "resume",
        revision=paused["revision"],
        key="resume-session-foreign-02",
        course_id="course-physics",
    )

    assert stale.status_code == 409
    assert stale.json()["detail"]["code"] == "study_session_control_conflict"
    assert reused.status_code == 409
    assert reused.json()["detail"]["code"] == "study_session_control_conflict"
    assert foreign.status_code == 404
    assert foreign.json()["detail"]["code"] == "study_session_not_found"


def test_history_lists_only_course_scoped_persisted_sessions(
    client: TestClient,
) -> None:
    _course_and_concept(client)
    _indexed_source(client)
    _task(client, task_id="task-history-first")
    _task(client, task_id="task-history-second")
    first = _start(client, "task-history-first").json()["session"]
    second = _start(client, "task-history-second").json()["session"]

    with client.app.state.database.connection() as connection:
        connection.execute(
            "UPDATE study_sessions SET updated_at = ? WHERE id = ?",
            ("2099-07-18T10:00:00+00:00", first["id"]),
        )
        connection.commit()
    ...
    assert all(item["course_id"] == COURSE_ID for item in body["sessions"])
    assert all("goal_scope" not in item for item in body["sessions"])
...
def test_start_api_creates_source_grounded_session_and_replays(
    ...
    assert all(
        unit["content"].startswith("Autonomous API concept cited source excerpt")
...
def test_start_api_persists_a_normalized_user_learning_goal(
    client: TestClient,
) -> None:
    _course_and_concept(client)
    _indexed_source(client)
    _task(client, task_id="task-api-user-goal")

    created = _start(
        client,
        "task-api-user-goal",
        goal="  Explain eigenvectors from my lecture notes  ",
    )

    assert created.status_code == 201
    session = created.json()["session"]
    assert session["goal"] == "Explain eigenvectors from my lecture notes"
    assert session["title"] == "Study: Explain eigenvectors from my lecture notes"

    replay = _start(
        client, "task-api-user-goal", goal="A later request must not replace it"
    )
    assert replay.status_code == 200
    assert replay.json()["session"]["goal"] == session["goal"]

    invalid = _start(client, "task-api-user-goal", goal="   ")
    assert invalid.status_code == 422


def test_user_goal_can_turn_a_covered_manual_task_into_guided_study(
    client: TestClient,
) -> None:
    _course_and_concept(client)
    _indexed_source(client)
    _task(client, task_id="task-api-covered-manual", autonomous=False)

    automated = _start(client, "task-api-covered-manual")
    started = _start(
        client,
        "task-api-covered-manual",
        goal="Understand this concept from my selected course",
    )

    assert automated.status_code == 200
    assert automated.json()["blocked_reason"] == "task_not_autonomous"
    assert started.status_code == 201
    assert started.json()["outcome"] == "session_created"
    assert started.json()["session"]["goal"] == (
        "Understand this concept from my selected course"
    )
    assert started.json()["session"]["originating_task_id"] == (
        "task-api-covered-manual"
    )
