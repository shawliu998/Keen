...
def test_studying_session_pause_survives_reopen_and_resumes_exact_phase(
    tmp_path,
) -> None:
    database = _database(tmp_path)
    with database.connection() as connection:
        ...
        begun = diagnostic.begin(
            ...
            idempotency_key="pause-reopen-begin-diagnostic",
            now=NOW,
        )
        answered = diagnostic.answer(
            ...
            idempotency_key="pause-reopen-answer-diagnostic",
            response="I can explain the chain rule from the saved source.",
        ...
        paused = StudySessionControlService(connection).apply(
            ...
            idempotency_key="pause-studying-after-reopen",
            now=NOW,
        )
        assert paused.session["status"] == "paused"
        assert paused.session["resume_from_status"] == "studying"
    ...
    with database.connection() as reopened:
        ...
        assert restored.session["resume_from_status"] == "studying"
        resumed = StudySessionControlService(reopened).apply(
            command="resume",
            course_id="course-calculus",
            session_id=session_id,
            expected_revision=int(restored.session["revision"]),
            idempotency_key="resume-studying-after-reopen",
            now=NOW,
        )

    assert resumed.session["status"] == "studying"
...
def test_unrelated_indexed_chunks_cannot_create_a_session_or_change_mastery(
    tmp_path,
) -> None:
    database = _database(tmp_path)
    with database.connection() as connection:
        ...
        connection.execute(
            ...
            SET content = CASE ordinal
                WHEN 0 THEN 'Mitochondria release chemical energy.'
                ELSE 'Ribosomes assemble cellular proteins.'
            END
            WHERE document_id = 'source-document'
            """
        )
        connection.commit()
        _recommendation_task(connection)
        mastery_before = dict(
            connection.execute(
                """
                SELECT probability, attempts, updated_at
                FROM mastery
                WHERE concept_id = 'concept-chain-rule'
                """
            ).fetchone()
        )
    ...
    assert tuple(persisted) == (0, 0, 0, *tuple(evidence_before))
        ...
        for ordinal in range(2):
            connection.execute(
                ...
                (
                    ...
                    ordinal,
                    f"Current chain rule source excerpt {ordinal + 1}.",
                    f"{ordinal + 10:064x}",
                    NOW.isoformat(),
                ),
            )
