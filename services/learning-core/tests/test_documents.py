...
def test_macos_pdf_worker_uses_parent_rss_monitor_not_sparse_address_limits(monkeypatch):
    applied: list[int] = []
...
def test_pdf_worker_announces_isolated_group_before_parsing(monkeypatch):
    ...
    monkeypatch.setattr(documents_module, "_start_pdf_worker_parent_watch", lambda: None)
    monkeypatch.setattr(documents_module, "_arm_pdf_worker_wall_timer", lambda: None)
    monkeypatch.setattr(documents_module.signal, "setitimer", lambda *_args: None)
    monkeypatch.setattr(
        documents_module,
        "_apply_pdf_worker_limits",
        lambda: (_ for _ in ()).throw(documents_module.DocumentParseError("stop")),
    )
...
def test_pdf_worker_wall_timer_kills_the_group(monkeypatch):
    ...
    assert timers == [
        (documents_module.signal.ITIMER_REAL, documents_module.PDF_PARSE_WALL_TIMEOUT_SECONDS)
    ]
    assert killed == [True]
...
def test_pdf_worker_group_cleanup_kills_descendant_after_leader_exits():
    ...
    class ExitedProcess:
        ...
        @staticmethod
        def kill():
            raise AssertionError("an exited leader must not be killed as a live process")
    ...
    try:
        ...
        assert documents_module._darwin_process_group_has_live_members(leader.pid) is False
    finally:
        try:
...
def test_pdf_import_persists_pages_chunks_locations_and_status_history(
    ...
    response = _upload(client, auth_headers, "biology.pdf", pdf_bytes, "application/pdf")
    ...
    assert document["parser"] == "pypdf/6.14.2;keen-chunker/1"
...
def test_duplicate_content_hash_reuses_the_indexed_document(client, auth_headers, pdf_bytes):
...
def test_concurrent_duplicate_imports_create_one_document(client, auth_headers, pdf_bytes):
...
def test_upload_rejects_extension_mime_and_content_mismatches(client, auth_headers, pdf_bytes):
    wrong_mime = _upload(client, auth_headers, "notes.pdf", pdf_bytes, "text/plain")
    unsupported = _upload(client, auth_headers, "notes.exe", b"plain text", "text/plain")
...
def test_upload_enforces_streaming_size_limit_and_cleans_temporary_file(tmp_path, auth_headers):
    settings = Settings(
        session_token=TOKEN,
...
def test_fts_search_and_grounded_query_return_verifiable_page_citations(
    ...
    imported = _upload(client, auth_headers, "biology.pdf", pdf_bytes, "application/pdf")
...
def test_markdown_sections_and_client_filename_never_control_storage_path(client, auth_headers):
    markdown = b"# Cell Biology\n\n## Mitochondria\n\nCristae contain the electron transport chain."
    response = _upload(client, auth_headers, "../../notes.md", markdown, "text/markdown")

    assert response.status_code == 201
    assert response.json()["document"]["name"] == "notes.md"
    search = client.post(
        "/v1/search",
        headers=auth_headers,
        json={"query": "Cristae"},
    ...
    assert search.status_code == 200
    assert search.json()["results"][0]["sectionPath"] == ["Cell Biology", "Mitochondria"]
    with client.app.state.database.connection() as connection:
        storage_path = connection.execute(
            "SELECT storage_path FROM document_versions"
        ).fetchone()["storage_path"]
...
def test_textless_pdf_persists_failed_state_and_removes_stored_file(
    ...
    assert detail["documentId"] is not None
    assert "OCR is not implemented" in detail["recovery"]
    ...
    assert not (client.app.state.settings.document_data_path / version["storage_path"]).exists()
...
def test_fts_query_with_only_operators_is_a_clear_validation_error(client, auth_headers):
...
def test_request_scoped_sqlite_connections_are_safe_under_concurrency(client, auth_headers):
...
def test_startup_marks_interrupted_import_failed_and_allows_retry(tmp_path, auth_headers):
    ...
    with TestClient(create_app(settings)) as recovered_client:
        ...
        assert retried.json()["document"]["status"] == "indexed"
