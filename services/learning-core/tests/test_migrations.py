...
def test_031_adds_durable_plan_decisions_and_scope_guards(tmp_path):
    database = Database(tmp_path / "plan-proposals.sqlite3")
    database.migrate()
    ...
    with database.connection() as connection:
        columns = {
            ...
            for row in connection.execute(
                "PRAGMA table_info(study_plan_proposals)"
            ).fetchall()
        }
        indexes = {
            row["name"]
            for row in connection.execute(
                "PRAGMA index_list(study_plan_proposals)"
            ).fetchall()
        }
        triggers = {
            row["name"]
            for row in connection.execute(
                ...
                WHERE type = 'trigger' AND tbl_name = 'study_plan_proposals'
                """
            ).fetchall()
        }
    ...
    assert {
        "study_plan_proposals_session_idx",
        "study_plan_proposals_base_plan_idx",
    }.issubset(indexes)
    assert {
        "study_plan_proposals_scope_insert",
        "study_plan_proposals_scope_update",
        "study_plan_proposals_versions_insert",
        "study_plan_proposals_versions_update",
    } == triggers


def test_028_preserves_existing_sessions_as_legacy_canonical(tmp_path):
    database = Database(tmp_path / "adaptive-forward.sqlite3")
    ...
    with database.connection() as connection:
        ...
        connection.execute(
            """INSERT INTO courses (id, title, description, created_at)
               VALUES ('legacy-course', 'Legacy', '', '2026-07-22T00:00:00+00:00')"""
        )
        connection.execute(
            """INSERT INTO study_sessions (
                   id, course_id, title, mode, goal, status, estimated_minutes,
                   created_at, updated_at
               ) VALUES (
                   'legacy-session', 'legacy-course', 'Legacy session', 'study',
                   'Keep canonical behavior', 'draft', 10,
                   '2026-07-22T00:00:00+00:00', '2026-07-22T00:00:00+00:00'
               )"""
        )
        connection.commit()

    assert database.migrate() == [28, 29, 30, 31, 32]
    with database.connection() as connection:
        session = connection.execute(
            "SELECT adaptive_policy_version FROM study_sessions WHERE id = 'legacy-session'"
        ).fetchone()
        table = connection.execute(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'study_adaptive_actions'"
        ).fetchone()
    assert session["adaptive_policy_version"] is None
    assert table["name"] == "study_adaptive_actions"


def test_029_adds_nullable_persisted_review_task_context(tmp_path):
    database = Database(tmp_path / "review-task-context-forward.sqlite3")
    migrations = Path(__file__).resolve().parent.parent / "migrations"
    with database.connection() as connection:
        connection.execute(
            """CREATE TABLE schema_migrations (
    ...
    assert database.migrate() == [29, 30, 31, 32]
    with database.connection() as connection:
        columns = {
            row["name"]: row
            for row in connection.execute(
                "PRAGMA table_info(review_attempts)"
            ).fetchall()
        }
        indexes = {
            row["name"]
            for row in connection.execute(
                "PRAGMA index_list(review_attempts)"
            ).fetchall()
        }
    assert columns["originating_task_id"]["notnull"] == 0
    assert {
        "review_attempts_originating_task_idx",
        "review_attempts_originating_task_unique",
    }.issubset(indexes)
...
def test_embedding_migration_is_forward_only_without_fabricating_legacy_vectors(
    ...
    assert applied[3:] == list(range(9, 33))
    with database.connection() as connection:
        assert (
            connection.execute(
...
def test_migration_007_forward_repairs_early_006_model_immutability(tmp_path):
    ...
    assert applied[2:] == list(range(9, 33))
    with database.connection() as connection:
...
def test_learning_loop_migrations_preserve_existing_008_learning_state(tmp_path):
    ...
    assert database.migrate() == list(range(9, 33))
    database.verify_consistency()
    with database.connection() as connection:
...
def test_032_adds_empty_outcome_lineage_without_fabricating_history(tmp_path):
    database = Database(tmp_path / "intervention-outcomes-forward.sqlite3")
    migrations = Path(__file__).resolve().parent.parent / "migrations"
    with database.connection() as connection:
        connection.execute(
            """CREATE TABLE schema_migrations (
    ...
    assert database.migrate() == [32]
    assert database.migrate() == []
    with database.connection() as connection:
        assert (
            connection.execute(
                "SELECT title FROM courses WHERE id = 'kept-lineage-course'"
            ).fetchone()[0]
            == "Kept lineage course"
        )
        assert (
