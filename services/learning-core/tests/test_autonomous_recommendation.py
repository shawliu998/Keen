...
@pytest.mark.parametrize(
    ("time_zone", "expected_scheduled", "expected_due"),
    [
        (
            "Asia/Shanghai",
            "2026-07-17T16:00:00+00:00",
            "2026-07-18T15:59:59.999999+00:00",
        ),
        (
            "America/Phoenix",
            "2026-07-17T07:00:00+00:00",
            "2026-07-18T06:59:59.999999+00:00",
        ),
    ],
)
def test_recommendation_uses_the_learner_iana_day_for_utc_storage(
    tmp_path, time_zone, expected_scheduled, expected_due
):
    database = _database(tmp_path)
    recommendation_time = datetime(2026, 7, 17, 20, 0, tzinfo=UTC)
    with database.connection() as connection:
        _due_review(
            connection,
            item_id="review-zoned",
            course_id="course-calculus",
            concept_id="concept-chain-rule",
        )
        result = AutonomousRecommendationCoordinator(connection).observe_and_recommend(
    ...
    assert result.outcome == "task_created"
    assert result.task is not None
    assert result.task["scheduled_for"] == expected_scheduled
    assert result.task["due_at"] == expected_due
    assert result.snapshot.as_of == recommendation_time


def test_completed_recommendation_replays_until_the_next_learner_local_day(tmp_path):
    database = _database(tmp_path)
    with database.connection() as connection:
        _due_review(
            connection,
            item_id="review-local-replay",
            course_id="course-calculus",
            concept_id="concept-chain-rule",
        )
        coordinator = AutonomousRecommendationCoordinator(connection)
        ...
        TaskRepository(connection).complete(
            ...
            completed_at="2026-07-17T21:00:00+00:00",
        )
        same_local_day = coordinator.observe_and_recommend(
            course_id="course-calculus",
            now=datetime(2026, 7, 18, 10, 0, tzinfo=UTC),
            available_minutes=30,
            time_zone="Asia/Shanghai",
        )
        next_local_day = coordinator.observe_and_recommend(
            course_id="course-calculus",
    ...
    assert same_local_day.outcome == "replay"
    assert same_local_day.task is not None
    assert same_local_day.task["id"] == first.task["id"]
    assert next_local_day.outcome == "task_created"
    assert next_local_day.task is not None
    assert next_local_day.task["id"] != first.task["id"]


def test_previous_bucket_task_completed_today_replays_without_a_new_task(tmp_path):
    database = _database(tmp_path)
    with database.connection() as connection:
        _insert_course(connection, "course-local-completed")
        connection.execute(
            """
            INSERT INTO concepts
                (id, course_id, name, bkt_slip, bkt_guess, bkt_transit)
            VALUES ('concept-local-completed', 'course-local-completed',
                    'Local completion', 0.1, 0.2, 0.1)
            """
        )
        connection.commit()
        _due_review(
            connection,
            item_id="review-completed-today",
            course_id="course-local-completed",
            concept_id="concept-local-completed",
        )
        coordinator = AutonomousRecommendationCoordinator(connection)
        previous_day = coordinator.observe_and_recommend(
            course_id="course-local-completed",
            now=datetime(2026, 7, 17, 10, 0, tzinfo=UTC),
            available_minutes=30,
            time_zone="Asia/Shanghai",
        )
        assert previous_day.task is not None
        TaskRepository(connection).complete(
            previous_day.task["id"],
            expected_revision=previous_day.task["revision"],
        ...
        count = connection.execute(
            """
            SELECT COUNT(*) FROM study_tasks
            WHERE course_id = 'course-local-completed'
              AND json_extract(priority_components_json, '$.candidate_id') = ?
            """,
            (replay.candidate.id,),
        ).fetchone()[0]
    ...
    assert replay.task["id"] == previous_day.task["id"]
    assert count == 1
    assert replay.snapshot.pending_tasks == ()
    assert [task["id"] for task in replay.snapshot.completed_tasks] == [
        previous_day.task["id"]
    ]
