...
def test_latest_agent_activity_returns_a_truthful_empty_or_persisted_public_snapshot(
    tmp_path,
):
    settings = _settings(tmp_path, seed_demo=True)
    provider = FixedAutomationProvider(
        [ContentDelta(text="Recorded activity."), ProviderFinished()]
    )
    with TestClient(
        create_app(settings, agent_provider_factory=lambda: provider)
    ) as client:
        assert client.get(
            "/v1/agent/runs/latest?conversationId=missing", headers=AUTH
        ).json() == {"run": None, "events": []}
        assert client.get("/v1/agent/runs/latest", headers=AUTH).status_code == 422
        assert (
            client.get(
                "/v1/agent/runs/latest?conversationId=one&studySessionId=two",
                headers=AUTH,
            ).status_code
            == 422
        )
        ...
        created = client.post(
            ...
            headers=AUTH,
            json={
                **_payload(key="agent-activity-latest"),
        ...
        response = client.get(
            "/v1/agent/runs/latest?conversationId=activity-conversation",
            headers=AUTH,
        )
        assert response.status_code == 200
        activity = response.json()
        assert activity["run"] == terminal
        assert activity["events"]
        assert [event["sequence"] for event in activity["events"]] == list(
            range(len(activity["events"]))
        )
        assert activity["events"][0]["type"] == "metadata"
        ...
        assert "userIntent" not in serialized
        assert "input" not in serialized


def test_latest_agent_activity_requires_authentication(tmp_path):
    with TestClient(create_app(_settings(tmp_path))) as client:
        assert (
            client.get("/v1/agent/runs/latest?conversationId=missing").status_code
            == 401
        )


def test_legacy_checkpoint_is_normalized_for_latest_and_sse_without_changing_approvals(
    tmp_path,
):
    settings = _settings(tmp_path, seed_demo=True)
    with TestClient(create_app(settings)) as client:
        with Database(settings.database_path).connection() as connection:
            ConversationRepository(connection).create_conversation(
                conversation_id="legacy-checkpoint-conversation",
                course_id="course-calculus",
                title="Legacy checkpoint",
            )
            repository = AgentRepository(connection)
            repository.create_run(
                run_id="legacy-checkpoint-run",
                conversation_id="legacy-checkpoint-conversation",
                kind="conversation",
                ...
                prompt_version="v1",
                input_data={},
                idempotency_key="legacy-checkpoint-run",
            )
            repository.transition_run("legacy-checkpoint-run", status="running")
            repository.append_event(
                event_id="legacy-raw-checkpoint",
                run_id="legacy-checkpoint-run",
                event_type="checkpoint",
                payload={"kind": "historical_completion", "artifactId": "artifact-1"},
            )
            approval = {
                ...
                "data": {
                    ...
                    "summary": {
                        "title": "Complete task",
                        "taskTitle": "Read limits",
                        "courseTitle": "Calculus",
                        "effect": "Marks one local task complete.",
                    },
                },
            }
            repository.append_event(
                event_id="legacy-approval-checkpoint",
                run_id="legacy-checkpoint-run",
                event_type="checkpoint",
                payload=approval,
            )
            repository.transition_run("legacy-checkpoint-run", status="completed")
            repository.append_event(
                event_id="legacy-checkpoint-done",
                run_id="legacy-checkpoint-run",
                event_type="done",
        ...
        latest = client.get(
            "/v1/agent/runs/latest?conversationId=legacy-checkpoint-conversation",
            headers=AUTH,
        )
        assert latest.status_code == 200
        latest_events = latest.json()["events"]
        assert latest_events[0]["data"] == {
            "label": "completion_published",
            "data": {"kind": "historical_completion", "artifactId": "artifact-1"},
        }
        assert latest_events[1]["data"] == approval

        stream = client.get("/v1/agent/runs/legacy-checkpoint-run/events", headers=AUTH)
        assert stream.status_code == 200
        sse_data = [json.loads(event["data"]) for event in _sse_events(stream.text)]
        assert sse_data[0] == latest_events[0]["data"]
        assert sse_data[1] == approval
