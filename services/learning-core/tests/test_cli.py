...
def test_sidecar_secret_frames_are_ordered_bounded_and_require_eof() -> None:
    api_key = "secret-key"
    payload = f"{len(TOKEN)}\n{TOKEN}{len(api_key)}\n{api_key}".encode()
    assert resolve_sidecar_secrets(
        expects_api_key=True, input_stream=BytesIO(payload)
    ) == (TOKEN, "secret-key")
    with pytest.raises(SystemExit, match="ended early"):
        resolve_sidecar_secrets(
            expects_api_key=True,
            input_stream=BytesIO(f"{len(TOKEN)}\n{TOKEN}9\nshort".encode()),
        )
    with pytest.raises(SystemExit, match="line breaks"):
        resolve_sidecar_secrets(
            expects_api_key=False, input_stream=BytesIO(b"4\na\r\nb")
        )
    with pytest.raises(SystemExit, match="trailing data"):
        resolve_sidecar_secrets(
            expects_api_key=False,
            input_stream=BytesIO(f"{len(TOKEN)}\n{TOKEN}x".encode()),
        )
