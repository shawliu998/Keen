...
def test_begin_exact_replay_survives_a_later_current_unit(tmp_path) -> None:
    ...
    with database.connection() as connection:
        ...
        assert len(units) >= 2
        connection.execute(
            "UPDATE study_sessions SET current_unit_id = NULL WHERE id = ?",
            (session_id,),
        )
        connection.execute(
            "UPDATE study_units SET status = 'completed' WHERE id = ?",
            (units[0]["id"],),
        )
        connection.execute(
        ...
        connection.execute(
            """UPDATE study_sessions
               SET status = 'active_recall', current_unit_id = ?, revision = revision + 1
               WHERE id = ?""",
            (units[1]["id"], session_id),
        )
        connection.commit()
...
def test_answer_applies_one_practice_bkt_event_not_fsrs_and_replays(
    ...
    with database.connection() as connection:
        ...
        later = StudyRepository(connection).transition_session(
            session_id,
    ...
    assert later["status"] == "review_scheduling"
    assert restored.outcome == "answered" and restored.grade == answered.grade
